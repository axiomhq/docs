    ...
    <details className="api-try">
      ...
      <div className="api-try-panel">
        ...
        <div className="api-try-fields">
          ...
          <label><span>Organization ID <em>Optional</em></span><input data-ph-no-capture type="text" autoComplete="off" value={orgId} onChange={(event) => persistCredential(orgKey, event.target.value, setOrgId)} placeholder="Your organization ID" /></label>
        ...
        <div className="api-try-actions"><button type="button" disabled={pending || !token} onClick={runRequest}>{pending ? <LoaderCircle className="api-try-spinner" size={13} /> : <Play size={12} />}{pending ? 'Running…' : `Run ${method.toUpperCase()} request`}</button></div>
