...
export function ApiTryIt({
  ...
  return (
    ...
    >
      <summary className="min-h-[46px] py-2 px-3 flex items-center gap-[9px] list-none [&::-webkit-details-marker]:hidden text-(--text-secondary) cursor-pointer hover:bg-interactive-hover group-open:bg-interactive-selected"><span className="api-try-chevron h-5 inline-flex items-center text-(--text-quaternary) transition-transform duration-150 ease-[ease] group-open:rotate-90"><ChevronRight size={14} /></span><span className="flex items-baseline gap-[9px]"><strong className="text-(--text-primary) font-mono text-[12px] leading-[18px] font-semibold">Try it</strong><small className="text-(--text-quaternary) font-sans text-[11px] leading-4 font-normal">Run this request against your Axiom organization</small></span></summary>
      <div className="api-try-panel pt-[14px] px-4 pb-4 border-t border-t-(--border-primary)">
        ...
        <div className="api-try-fields grid grid-cols-2 gap-3 max-sm:grid-cols-1">
          ...
          {visibleParameters.map((item) => <label className="min-w-0 flex flex-col gap-1.5" key={`${item.location}-${item.name}`}><span className="text-(--text-secondary) font-mono text-[11px] leading-4 font-medium">{item.name} <code className="ml-1.5 p-0 border-0 bg-transparent text-(--text-quaternary) font-mono text-[10px] leading-[14px] font-medium not-italic tracking-[.04em] uppercase">{item.location}</code>{item.required && <b className="ml-1.5 text-(--color-warning-text) font-mono text-[10px] leading-[14px] font-medium not-italic tracking-[.04em] uppercase">Required</b>}</span><input className="w-full border border-(--border-primary) rounded-md outline-none text-(--text-primary) bg-(--bg-canvas) font-mono! text-[12px]! leading-[18px]! font-normal! focus:border-(--color-accent) focus:shadow-[0_0_0_2px_color-mix(in_srgb,var(--color-accent)_18%,transparent)] h-[34px] px-2.5 py-0 placeholder:text-(--text-quaternary)" value={values[item.name] ?? ''} onChange={(event) => setValues((current) => ({ ...current, [item.name]: event.target.value }))} placeholder={item.description} /></label>)}
          {bodyType && <label className="api-try-body col-span-full max-sm:col-auto min-w-0 flex flex-col gap-1.5"><span className="text-(--text-secondary) font-mono text-[11px] leading-4 font-medium">Request body <code className="ml-1.5 p-0 border-0 bg-transparent text-(--text-quaternary) font-mono text-[10px] leading-[14px] font-medium not-italic tracking-[.04em] uppercase">{bodyType}</code></span><textarea className="w-full border border-(--border-primary) rounded-md outline-none text-(--text-primary) bg-(--bg-canvas) font-mono! text-[12px]! leading-[18px]! font-normal! focus:border-(--color-accent) focus:shadow-[0_0_0_2px_color-mix(in_srgb,var(--color-accent)_18%,transparent)] min-h-[150px] p-2.5 resize-y" value={body} onChange={(event) => setBody(event.target.value)} spellCheck={false} /></label>}
        </div>
