    ...
    <footer className="article-footer mt-20 text-(--text-quaternary) font-mono text-xs leading-4 font-[450]">
      ...
      {(previous || next) && (
        <nav className="article-pagination mt-8 grid grid-cols-2 gap-6 max-sm:grid-cols-[1fr]" aria-label="Adjacent documentation pages">
          {previous && (
            <Link href={previous.href} prefetch={false} className={cn('article-previous items-start', 'min-w-0 -mx-3.5 px-3.5 py-3 flex flex-col-reverse gap-1.5 rounded-[6px] no-underline transition-[background-color] duration-150 ease-[ease] [&:hover]:bg-interactive-hover')}>
          ...
          {next && (
            <Link href={next.href} prefetch={false} className={cn('article-next col-[2] items-end text-right max-sm:col-[1]', 'min-w-0 -mx-3.5 px-3.5 py-3 flex flex-col-reverse gap-1.5 rounded-[6px] no-underline transition-[background-color] duration-150 ease-[ease] [&:hover]:bg-interactive-hover')}>
              <small className="inline-flex items-center gap-1.5 text-(--text-quaternary) font-mono text-[10px] leading-[14px] font-medium tracking-[.05em] uppercase transition-colors duration-150 ease-[ease] [.article-pagination_a:hover_&]:text-(--text-tertiary)">
                Next
