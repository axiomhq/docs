...
const placeholderControlClass = 'min-w-0 h-8 px-2.5 py-0 border border-(--border-primary) rounded-[4px] outline-none text-(--text-secondary) bg-(--bg-canvas) font-mono text-[12px] leading-4 font-normal focus:border-(--blue-500) focus:shadow-[0_0_0_2px_rgba(59,130,246,.15)]';
...
export function PlaceholderPre({ source: sourceProp, ...props }: ComponentProps<'pre'> & { source?: string }) {
  ...
  return <>
    ...
    {hydrated && usedFields.length > 0 && <div className="placeholder-config ph-no-capture mt-0 mx-0 mb-6 py-[14px] px-4 flex flex-col gap-2.5 border-x border-b border-(--border-primary) rounded-t-none rounded-b-[4px] bg-(--bg-surface)" aria-label="Customize this example" data-ph-no-capture>
      {usedFields.map((key) => {
        const field = fields[key];
        return <label className="grid grid-cols-[110px_1fr] items-center gap-3 text-(--text-tertiary) font-sans text-[12px] leading-4 font-medium max-sm:grid-cols-[1fr] max-sm:gap-[5px]" key={key}><span>{field.label}</span>{'options' in field ? <select className={placeholderControlClass} value={values[key] ?? ''} onChange={(event) => update(key, event.target.value)}>{field.options.map((option) => <option value={option} key={option}>{option || 'Select edge deployment'}</option>)}</select> : <input className={cn(placeholderControlClass, 'font-mono! text-[12px]! leading-4! font-normal!')} value={values[key] ?? ''} placeholder={field.placeholder} autoComplete="off" data-1p-ignore data-ph-no-capture onChange={(event) => update(key, event.target.value)} />}</label>;
      })}
    </div>}
  </>;
}
