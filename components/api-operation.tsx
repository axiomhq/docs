...
function SchemaTable({ document, rows, label }: { document: JsonObject; rows: SchemaRow[]; label: string }) {
  ...
  return (
    <div className="api-schema-wrap mt-[14px] overflow-x-auto border border-(--border-primary) rounded-md max-sm:-mr-2">
      ...
      <table className="api-schema-table w-full min-w-[620px] m-0! border-0! border-collapse table-fixed text-[13px]! [&_th:last-child]:border-r-0! [&_td:last-child]:border-r-0! [&_thead_th:first-child]:rounded-tl-[5px] [&_thead_th:last-child]:rounded-tr-[5px] [&_tbody_tr:last-child_td:first-child]:rounded-bl-[5px] [&_tbody_tr:last-child_td:last-child]:rounded-br-[5px]" data-has-location={hasLocation || undefined} aria-label={label}>
        <thead><tr><th className={cn('py-2 px-2.5 border-t-0! border-l-0! border-r! border-r-(--border-primary)! border-b! border-b-(--border-primary)! text-(--text-secondary) bg-(--bg-inert) font-mono text-[13px] leading-[18px] font-normal text-left', hasLocation ? 'w-[28%]' : 'w-[30%]')}>Property</th><th className={cn('py-2 px-2.5 border-t-0! border-l-0! border-r! border-r-(--border-primary)! border-b! border-b-(--border-primary)! text-(--text-secondary) bg-(--bg-inert) font-mono text-[13px] leading-[18px] font-normal text-left', hasLocation ? 'w-[18%]' : 'w-[25%]')}>Type</th>{hasLocation && <th className="py-2 px-2.5 border-t-0! border-l-0! border-r! border-r-(--border-primary)! border-b! border-b-(--border-primary)! text-(--text-secondary) bg-(--bg-inert) font-mono text-[13px] leading-[18px] font-normal text-left w-[14%]">Location</th>}<th className="py-2 px-2.5 border-t-0! border-l-0! border-r! border-r-(--border-primary)! border-b! border-b-(--border-primary)! text-(--text-secondary) bg-(--bg-inert) font-mono text-[13px] leading-[18px] font-normal text-left">Description</th></tr></thead>
        <tbody className="[&>tr:last-child>td]:border-b-0!">{rows.map((row) => (
          <tr className={row.hasChildren ? 'api-schema-object-row [&>td]:bg-(--bg-inert)' : undefined} data-depth={row.depth} key={row.key}>
            <td className="api-schema-name relative whitespace-nowrap py-[9px] px-2.5 border-t-0! border-l-0! border-r! border-r-(--border-tertiary)! border-b! border-b-(--border-tertiary)! align-top" style={{ paddingLeft: 10 + row.depth * 18 }}>
              ...
              <code className="p-0! border-0! bg-transparent! font-mono text-(--text-primary)! text-[13px]! font-[550]">{row.name}</code>
            </td>
            ...
            {hasLocation && <td className="api-schema-location py-[9px] px-2.5 border-t-0! border-l-0! border-r! border-r-(--border-tertiary)! border-b! border-b-(--border-tertiary)! align-top"><code className="p-0! border-0! text-(--text-tertiary)! bg-transparent! font-mono! text-[10px]! leading-[14px]! font-medium! tracking-[.05em] uppercase">{row.location}</code></td>}
            <td className="api-schema-description text-(--text-secondary)! font-sans text-[13px] leading-[22px] font-normal py-[9px] px-2.5 border-t-0! border-l-0! border-r! border-r-(--border-tertiary)! border-b! border-b-(--border-tertiary)! align-top"><PlainMarkdown value={row.description} codeClassName="p-0! border-0! rounded-none! bg-transparent! font-mono text-[length:inherit]!" />{!row.description && <span aria-hidden="true">—</span>}</td>
          </tr>
        ))}</tbody>
      </table>
    </div>
...
async function highlightedSamples(samples: ReturnType<typeof requestSamples>): Promise<ApiCodeSample[]> {
  return Promise.all(samples.map(async (sample) => ({
    ...
    highlighted: await highlight(sample.code, { lang: sample.language, themes: { light: axiomCodeLight, dark: axiomCodeDark }, defaultColor: false }),
  })));
}
...
export async function ApiOperation({ value, children }: { value: string; children: ReactNode }) {
  ...
  return <div className="api-operation">
    <div className="endpoint-bar my-4 min-h-[38px] py-1.5 pl-3 pr-1.5 flex items-center gap-2.5 border border-(--border-primary) rounded-md bg-(--bg-surface)">
      <span className={cn('endpoint-method min-w-[34px] flex-none rounded-[3px] px-1 py-px text-center font-mono text-[10px] leading-[14px] font-normal tracking-[.02em]', METHOD_COLORS[method] ?? 'method-' + method)}>{method.toUpperCase()}</span>
      {/* `!` beats the prose inline-code chip styles this <code> sits inside. */}
      <code className="flex-1 overflow-x-auto text-(--text-primary) bg-transparent! border-0! font-mono! text-[13px]! leading-[18px]! font-[450]! whitespace-nowrap">{displayPath}</code>
      <CopyButton value={displayPath} label="" analytics={{ codeKind: 'endpoint_path' }} className="h-6 w-6 p-0 flex-none justify-center rounded text-(--text-tertiary) active:scale-90 hover:text-(--text-primary) dark:hover:bg-[#232323]!" />
    </div>
    ...
    {children}
    {parameters.length > 0 && <section className="api-section mt-11"><HeadingAnchor as="h2" id="parameters" className="m-0! p-0 border-0 text-(--text-primary) font-sans! text-[18px]! leading-6! font-semibold! tracking-[-.01em]">Parameters</HeadingAnchor><SchemaTable document={document} rows={parameterRows} label="Request parameters" /></section>}
    {bodyContent && <section className="api-section mt-11"><HeadingAnchor as="h2" id="body" className="m-0! p-0 border-0 text-(--text-primary) font-sans! text-[18px]! leading-6! font-semibold! tracking-[-.01em]">Body</HeadingAnchor><p className="api-section-copy mt-3 text-(--text-secondary) text-[15px] leading-[25px]"><PlainMarkdown value={resolveSchema(document, bodySchema)?.description ?? operation.requestBody?.description} codeClassName="py-px px-1 border border-(--border-secondary) rounded-[3px] bg-(--bg-inert) font-mono" /></p><div className="media-types mt-3 flex flex-wrap gap-1.5">{Object.keys(bodyContent).map((type) => <code className="py-[3px] px-[7px] border border-(--border-primary) rounded-[3px] text-(--text-tertiary) bg-(--bg-inert) text-[10px]" key={type}>{type}</code>)}</div><SchemaTable document={document} rows={bodyRows} label="Request body schema" /></section>}
    <section className="api-section mt-11"><HeadingAnchor as="h2" id="example" className="m-0! p-0 border-0 text-(--text-primary) font-sans! text-[18px]! leading-6! font-semibold! tracking-[-.01em]">Request</HeadingAnchor><ApiCodeBlock samples={requestCodeSamples} /><ApiTryIt operation={value} method={method} parameters={tryParameters} serverVariables={serverVariables} bodyType={bodyType} bodyExample={bodyExample} /></section>
    {responseEntries.length > 0 && <section className="api-section mt-11"><HeadingAnchor as="h2" id="response" className="m-0! p-0 border-0 text-(--text-primary) font-sans! text-[18px]! leading-6! font-semibold! tracking-[-.01em]">Response</HeadingAnchor>{responseEntries.map(([code, response], index) => <div className={cn('api-response min-h-[43px] py-2.5 px-1 flex items-center gap-3 border-b border-b-(--border-tertiary) max-sm:items-start max-sm:flex-col', index === responseEntries.length - 1 && 'border-b-0!')} key={code}><div className="flex items-center gap-2"><span className={cn(/^2/.test(code) ? 'status-dot success bg-(--green-500)' : 'status-dot error bg-(--red-400)', 'size-2 inline-block rounded-[999px] text-(--text-tertiary) text-[12px]')} /><code className="p-0! border-0! bg-transparent! text-(--text-secondary)!">{code}</code><span className="text-(--text-tertiary) text-[12px]">{response.description}</span></div>{response.content && <small className="ml-auto text-(--text-quaternary) font-mono text-[10px] leading-[14px] font-[450] uppercase max-sm:ml-4">{Object.keys(response.content).join(', ')}</small>}</div>)}{responseSchema && <SchemaTable document={document} rows={responseRows} label="Response schema" />}</section>}
  </div>;
}
