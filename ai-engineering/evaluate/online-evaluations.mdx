...
Online evaluations build on Axiom AI SDK's instrumentation. Before adding online evals, ensure you have:

- Axiom AI SDK installed and configured with a tracer. See [Quickstart](/ai-engineering/quickstart) for the full setup, including OpenTelemetry configuration and environment variables.
- An AI model wrapped with `wrapAISDKModel` for automatic tracing. See [Instrumentation with Axiom AI SDK](/ai-engineering/observe/axiom-ai-sdk-instrumentation) for details.

## Getting started

Import `onlineEval` and `Scorer` from the Axiom AI SDK, and use your wrapped model:
