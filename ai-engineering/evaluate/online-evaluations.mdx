...
Online evaluations build on Axiom AI SDK's instrumentation. Before adding online evaluations:

- Install Axiom AI SDK and configure a tracer. See [Quickstart](/ai-engineering/quickstart) for the full setup, including OpenTelemetry configuration and environment variables.
...
Online evaluations use the same `Scorer` factory and return types (boolean, numeric, or `{ score, metadata }`) as offline evaluations. The key difference is that online scorers are reference-free. They receive `input` and `output` but no `expected` value.

For the full `Scorer` API, including boolean scorers, numeric scorers, and scorers with metadata, see [Write evaluations: Creating scorers](/ai-engineering/evaluate/write-evaluations#creating-scorers).
