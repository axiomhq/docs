...
LLM outputs are inherently non-deterministic. Even with temperature set to zero, the same input can produce different outputs across runs. This variability can make evaluation results noisy and hard to interpret.
...
Each case runs independently `trials` times. Scores from each trial are then aggregated according to the scorer's aggregation strategy.
