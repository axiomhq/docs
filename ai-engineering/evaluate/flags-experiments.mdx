...
AI capabilities have many tunable parameters: which model to use, what temperature setting, which prompting strategy, how to structure retrieval, and more. Without flags, you’d need to:
...
export async function classifyTicket(input: { subject?: string; content: string }) {
  ...
  const temperature = flag('ticketClassification.temperature');
...
Tell your evaluation which flags it depends on using `pickFlags()`:
...
axiom eval --flag.ticketClassification.temperature=0.3
...
```bash
# Baseline: default flags (gpt-4o-mini, temp 0.7, concise)
...
# Experiment 2: Lower temperature for more consistency
axiom eval spam-classification --flag.ticketClassification.temperature=0.2

# Experiment 3: Detailed prompting
...
ticketClassification: z.object({
  ...
  // Default to the approach that works best in production
  systemPrompt: z.enum(['concise', 'detailed']).default('concise'),
}),
```
