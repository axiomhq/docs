...
Scorers evaluate your capability's output. They receive the `input`, `output`, and `expected` values, and return a score (a number between 0-1, or boolean). The same `Scorer` API works in both offline and [online evaluations](/ai-engineering/evaluate/online-evaluations/write-run-evaluations). Online scorers are reference-free, so they only use `input` and `output`.
