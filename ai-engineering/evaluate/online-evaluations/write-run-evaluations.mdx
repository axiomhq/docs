...
Online evaluations use the same `Scorer` factory and return types (boolean, numeric, or `{ score, metadata }`) as [offline evaluations](/ai-engineering/evaluate/write-evaluations#create-scorers). The key difference is that online scorers are reference-free. They receive `input` and `output` but no `expected` value.
