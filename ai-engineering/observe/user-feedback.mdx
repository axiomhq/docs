...
The Axiom AI SDK supports several feedback types:
...
- Axiom AI SDK installed in your project. For more information, see [Quickstart](/ai-engineering/quickstart).
- An API token with ingest permissions. Since this token will be exposed in the frontend, it is recommended to scope it only to your feedback dataset.
- A dataset to store feedback events.

<Note>
Feedback events are stored separately from trace data. Create a dedicated dataset for feedback to keep these signals organized.
</Note>
...
const { sendFeedback } = createFeedbackClient({
  ...
  url: process.env.AXIOM_URL, // Optional, defaults to https://api.axiom.co
});
```
...
```bash .env
AXIOM_FEEDBACK_TOKEN="xaat-abc123"
AXIOM_FEEDBACK_DATASET="my-feedback-prod"
...
By default, errors are logged to the console. Pass an `onError` callback to handle errors differently:
...
<Warning>
When using feedback on the client side, the API token is exposed to users. Create a token with minimal permissions (ingest only, scoped to the feedback dataset).
</Warning>

## Example: chat with feedback
