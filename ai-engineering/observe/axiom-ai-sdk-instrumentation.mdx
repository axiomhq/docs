...
<Note>
When you use Axiom AI SDK with isolated execution environments like [Convex DB Actions](https://docs.convex.dev/functions/actions), export spans using `SimpleSpanProcessor` instead of `BatchSpanProcessor`. `SimpleSpanProcessor` exports spans immediately when they complete, which is necessary for environments where the process may terminate before batched spans are flushed.
</Note>
