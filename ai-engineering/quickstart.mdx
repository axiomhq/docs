...
1. Create an instrumentation file:
    ...
    export const setupAppInstrumentation: AxiomEvalInstrumentationHook = async ({
      ...
      // Use edgeUrl for trace ingestion to send data to the correct edge deployment
...
export default defineConfig({
  eval: {
    ...
    instrumentation: ({ edgeUrl, token, dataset }) =>
      setupAppInstrumentation({ edgeUrl, token, dataset }),
  },
});
```
...
```bash .env
AXIOM_EDGE_URL="AXIOM_EDGE_DOMAIN"
...
Replace `AXIOM_EDGE_DOMAIN` with `https://` followed by the base domain of your edge deployment. For example, `https://us-east-1.aws.edge.axiom.co` for US East 1 or `https://eu-central-1.aws.edge.axiom.co` for EU Central 1. For more information, see [Edge deployments](/reference/edge-deployments).
