...
1.  **Prototype a capability**: Development starts by defining a task and prototyping a `capability` with a prompt to solve it.
2.  **Evaluate with ground truth**: The prototype is then tested against a `collection` of reference examples (`ground truth`) to measure its quality and effectiveness using `graders`. This process is known as an `eval`.
3.  **Observe in production**: Once a `capability` meets quality benchmarks, it's deployed. In production, `graders` can be applied to live traffic (`online evals`) to monitor performance and cost in real-time.
4.  **Iterate with new insights**: Insights from production monitoring reveal edge cases and opportunities for improvement. These new examples are used to refine the `capability`, expand the `ground truth` `collection`, and begin the cycle anew.

## Rudder terms

### Capability

A generative AI `capability` is a function that uses AI models to perform a specific task by transforming inputs into desired outputs.

Capabilities exist on a spectrum of complexity. They can be a simple, single-step function (e.g., classifying a support ticket's intent) or evolve into a sophisticated, multi-step agent that uses reasoning and tools to achieve a goal (e.g., orchestrating a complete customer support resolution).

### Collection

A `collection` is a curated set of reference `records` organized around a specific `capability`, used for development, testing, and evaluation. `Collections` serve as the test cases for prompt engineering.
...
* Now that you understand the core concepts, see them in action in the [Rudder workflow](./TODO-rudder-workflow).