...
* **[Upcoming]** A dedicated **AI Trace Waterfall** view will visualize single and multi-step LLM workflows, with clear input/output inspection at each stage.
* **[Upcoming]** A pre-built **Gen AI OTel Dashboard** will automatically appear for any dataset receiving AI telemetry. It will feature widgets for tracking cost per invocation, time-to-first-token, call counts by model, and error rates.
