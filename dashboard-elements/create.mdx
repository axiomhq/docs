...
For dashboard elements created with APL, it is also possible to define the custom time range in the APL query.
...
Axiom displays <img src="/doc-assets/icons/clock.svg" className="inline-icon" alt="Clock icon" /> in the top left of the dashboard element to indicate that its time range is defined in APL and might be different from the dashboard’s time range.