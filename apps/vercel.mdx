...
If you use the Axiom Vercel integration, [annotations](/query-data/annotate-charts) are automatically created for deployments.

## What is Vercel?

Vercel is a platform for frontend frameworks and static sites, built to integrate with your headless content, commerce, or database.

Vercel provides a frictionless developer experience to take care of the complex tasks: deploying instantly, scaling automatically, and serving personalized content around the globe.
...
Simply install the [Axiom Vercel app from here](https://vercel.com/integrations/axiom) and start streaming logs and web vitals within minutes!
...
As users interact with your website, various logs are produced. Axiom captures all these logs and ingests them into the `vercel` dataset. You can stream and analyze these logs live or use our pre-built Vercel Dashboard to get an overview of all the important metrics. When you're ready, you can fork our dashboard and start building your own!

For function logs, if you call `console.log`, `console.warn`, or `console.error` in your function, the output is also captured and made available as part of the log. You can use our extended query language, APL, to easily search these logs.

## Web vitals

Axiom supports capturing and analyzing Web Vital data directly from your users' browsers without any sampling and with more data than is available elsewhere. It pairs perfectly with Vercel's built-in analytics when you want to get deep into a specific problem or debug issues with a specific audience (user-agent, location, region, etc.).

<Note>
Web Vitals are currently supported only for Next.js websites. Expanded support is coming soon.
</Note>

### Installation

Follow these steps to install Web Vitals:

1. In your Vercel project, run `npm install --save next-axiom`.
2. In `next.config.js`, wrap your Next.js config in `withAxiom` as follows:
...
- To send logs from different parts of your app, use the provided logging functions. For example:
...
If you're using Next.js version 12, follow these instructions to integrate Axiom for logging and capturing Web Vitals data.

In your `pages/_app.js` or `pages/_app.ts`, add the following line:
...
If you plan to upgrade to Next.js 13, make these changes to ensure compatibility:
...
- For server-side components, create an instance of the `Logger` and flush the logs before the component returns
- For Web Vitals tracking, replace the previous method of capturing data: remove the `reportWebVitals()` line and integrate the `AxiomWebVitals` component into your layout

## Vercel Function logs 4KB limit

The Vercel 4KB log limit is a restriction placed by Vercel on the size of log output generated by serverless functions running on their platform. The 4KB log limit means that each log entry produced by your function should be at most 4 kilobytes in size.

If your log output is larger than 4KB, you might experience truncation or missing logs. To log above this limit, send your function logs using [next-axiom](https://github.com/axiomhq/next-axiom).
...
5. In your terminal, go to the root folder of your Next.js app, and run `npm install --save next-axiom` to install the latest version of next-axiom.
