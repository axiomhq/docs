...
resource "axiom_monitor" "test_monitor" {
  ...
  # - "Threshold": For numeric values against thresholds (requires operator, threshold)
  # - "MatchEvent": For detecting specific events (doesn't require operator/threshold)
  # - "AnomalyDetection": For detecting anomalies (requires compare_days, tolerance, operator)
  ...
  # Required for Threshold and AnomalyDetection types
  operator         = "Above"  # Valid values: "Above", "AboveOrEqual", "Below", "BelowOrEqual"
  
  range_minutes    = 5
  
  # Required for Threshold type
  threshold        = 1
  
  # Required for AnomalyDetection type (uncomment if using that type)
  # compare_days     = 7
  # tolerance        = 25
