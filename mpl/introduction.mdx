---
title: Introduction to MPL
description: This page explains how to use MPL (Metrics Processing Language) to query metrics.
sidebarTitle: Language features
...
If you omit `to <time_window>`, `align` aggregates over the full query range and returns a single value per series. 
...
`compute` uses strict intersection semantics. It only combines series with matching tag sets, and it only emits values for timestamps present on both sides. If you need time-wise union-like behavior for matching series, fill missing values before applying `compute`.
