...
You can also start Spotlight from a table view by right-clicking on the value you want to select and choosing 'Run spotlight'.
...
For each visualization, the proportion of selected and baseline events where this field is present is also shown.

### Digging deeper

You can iteratively refine your Spotlight analysis, or jump to a view of matching events to dig deeper.

1. **Filter and re-run**: Right-click on specific values in the results and select 'Re-run spotlight' to filter your data and run Spotlight again with a more focused scope.
1. **Show events**: Rick-click on specific values in the results and select 'Show events' to filter your data and see matching events.

## Spotlight limitations

- **Custom attributes**: Currently, custom attributes in OTel spans are not included in the Spotlight results. Spotlight will be expanded to include custom attributes soon.
- **Complex queries**: Spotlight works when starting from a query with at most one aggregation step. Complex queries with multiple aggregations may not be supported.

## Example workflows

### Investigating slow traces

1. Create a heatmap query: `['traces'] | summarize histogram(duration, 20) by bin_auto(_time)`
...
### Understanding error spikes

1. Create a line chart: `['logs'] | where level == "error" | summarize count() by bin_auto(_time)`
