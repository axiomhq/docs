...
You can query metric data using two approaches:
...
- **MPL**: A text-based query language (Metrics Processing Language) for writing expressive queries directly. For more information, see [MPL reference](/query-data/metrics/mpl) and [Sample queries](/query-data/metrics/sample-queries).
...
If you use PromQL, you can translate your existing queries to MPL with minimal changes. MPL also offers features not available in PromQL, such as native support for non-string tag types and custom transformation pipelines.
