...
- **MPL**: A text-based query language (Metrics Processing Language) for writing expressive queries directly. For more information, see [Query metrics using MPL](/query-data/metrics/query-metrics-mpl) and [Sample queries](/query-data/metrics/sample-queries).
    <Info>
...
OpenTelemetry metrics use temporality to define how data points relate to each other over time. The two types of temporality are:

- **Delta temporality**: Each data point represents the change since the last report. For example, a counter with delta temporality reports the number of requests in the last interval.
- **Cumulative temporality**: Each data point represents the total accumulated value since a fixed start time. For example, a counter with cumulative temporality reports the total number of requests since the process started.

Axiom accepts both delta and cumulative temporality for all supported metric types. When choosing between them, consider the following:

- **Delta temporality** is generally recommended for most use cases. It's more efficient because each data point is smaller and independent, making it easier to aggregate across instances. The OpenTelemetry Collector defaults to delta temporality for OTLP exporters.
- **Cumulative temporality** can be useful when you want to track absolute totals or when your existing instrumentation already uses it (for example, Prometheus-style metrics).

<Note>
For histograms specifically, some query features like quantile calculations using `interpolate_histogram` are only supported with delta temporality. If you plan to compute percentiles from histogram data, use delta temporality.
</Note>

To configure temporality in the OpenTelemetry Collector, set the `temporality_preference` option in your OTLP exporter configuration:
