...
MPL treats regular expressions as a first-class type. They're written directly without string escaping, making them easier to use and less ambiguous.
...
```kusto MPL equivalent
{
    ...
    | filter resource == "cpu"
    | group by pod using sum;
} compute by pod using / as cpu_saturation
