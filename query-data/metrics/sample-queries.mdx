...
Calculate the weekly average ingest pressure for a set of services.
...
| where code == /[123]../
| bucket by method, path to 5m using interpolate_histogram(0.90, 0.99)
```
...
| bucket by handler to 30s using interpolate_histogram(0.5, 0.99)
```
