...
<Tip>
`where` filters series by their tag values. To filter individual data points within a series by their numeric values, use [`filter::` functions](#filter-functions) inside a `map` step.
</Tip>
...
The time window must be an explicit duration such as `5m` or `1h`. MPL does not support an automatic window size. If you omit `align`, Axiom returns raw data points without time-based aggregation.
...
Combine multiple subqueries into a single result. Each subquery can target the same metric or different metrics within the same dataset. This is how you express ratio calculations, deltas, and other cross-series comparisons in MPL.

<Info>
`compute` produces one named output series. If you need several independent series in one view, run separate queries.
</Info>
...
The following example queries the same metric (`http.server.request.duration`) twice with different filters to compute an error rate. Both subqueries target the same timeseries; the first applies a status code filter to isolate errors.
