...
- You can only query single metrics.
- You can only use bucketing for histograms.
...
This example queries the `k8s` dataset’s `cpu_usage` metric one hour before the current time. It filters results to events where `namespace` is `prod` and aggregates events over 5-minute time windows.
...
| `fill_const` | Fills missing values with a constant.                        |
...
Currently, only the `histogram` function is supported.
...
| bucket by project, namespace using wasm::ai::my_function(5m)
```
...
Use the `set` command to define settings at the beginning of a query.
...
To import custom functions to your query, use the `wasm` command.
