...
1. **Transformation** can be the following:
    ...
    - **Aggregation:** Aggregates the data over a given time.
    - **Grouping:** Groups the data by a set of tags, combining values over different series that are measured at the same timestamp.

## Build metrics query

1. Click the Query tab.
1. Click **Builder** in the top left.
1. In **Dataset**, select a OTel metrics dataset, and then select the metric you want to query.
1. To filter data:
