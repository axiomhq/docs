...
When the underlying metric is a histogram, `bucket` aggregations are computed using the `interpolate_histogram` function. For quantile aggregations, this will estimate the quantile using the underlying histogram data. Currently, `interpolate_histogram` is only supported for histograms that use delta temporality. 
...
| `quantile`    | Computes the specified quantile per group and time inverval. Any value between 0 and 1 can be chosen. You can use `quantile` to compute percetiles in your data. For example, P99 corresponds to the 0.99 quantile.         |
