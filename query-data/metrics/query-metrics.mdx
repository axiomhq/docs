...
When metrics emit data at regular intervals but with different offsets across time series, `align` maps your data to a fixed set of time intervals. This ensures data from different sources is easy to visualize on a single chart. Gaps appear when the alignment interval is shorter than the emission interval. Query over a longer time range or use a longer alignment interval to reduce gaps.
