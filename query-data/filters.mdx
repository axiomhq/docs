...
1. In the **Value** section, define the list of options to choose from in the select filter as key-value pairs. Axiom displays the key in the list of options in the filter dropdown, and uses the value to filter your data. For example, the key `France` is displayed in the list of options, and the value `FR` is used to filter data in your charts. Define the key-value pairs in one of the following ways:
    ...
    - Choose **Query** to define a dynamic list of options. In this case, Axiom determines the list of options displayed in the filter dynamically based on a query. You can use an APL query or an MPL metrics query:
        - For an APL query, the results must contain two fields which Axiom interprets as key-value pairs. Use the `project` command to create key-value fields from any output. See the example below.
        - For an MPL metrics query, group by a single tag. Axiom uses the distinct values of that tag as the filter options. For more information, see [Generate filter options from a metrics query](#generate-filter-options-from-a-metrics-query).
...
1. Create a dashboard element based on an APL query:
...
## Use filters with metrics queries

Filter bars work with MPL metrics queries as well as APL queries.

### Generate filter options from a metrics query

In addition to APL queries, you can populate a select filter’s dynamic list of options from an MPL metrics query. This lets you build the list of options from the tag values present in your metrics data.

To create a select filter whose options come from a metrics query:

1. Create a select filter as described in [Create select filters](#create-select-filters).
1. In the **Value** section, choose **Query**.
1. Enter an MPL query that groups by a single tag. Axiom uses the distinct values of that tag as the filter options. Group by exactly one tag. If the query groups by zero or several tags, Axiom can’t determine the filter options.
...
The MPL query below populates the filter options with the distinct values of the `service.name` tag in the `http.server.request.duration` metric:
...
<Note>
Only APL and MPL queries are supported for generating dynamic filter options. Structured queries aren’t supported.
</Note>
...
1. Create a select filter as described in [Create select filters](#create-select-filters).
1. In the filter options, check the **Unset** box for the default value.

Checking **Unset** tells Axiom to treat the variable as optional. When no option is selected, the variable is not passed to the query.
