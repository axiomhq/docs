...
In an MPL query, reference a filter by prefixing its ID with `$`. For example, a filter with ID `service_filter` is referenced as `$service_filter`. Axiom automatically passes the selected value as a parameter when the query runs — you don't need to declare anything manually.
...
The `$__interval` variable is a built-in that Axiom sets to the dashboard's current time interval. Use it in `align` to match the chart's resolution.
