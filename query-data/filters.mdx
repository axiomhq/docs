...
<Note>
Dashboard filters work with APL queries only. If a dashboard element uses Metrics Query Language (MPL), the filter bar doesn't apply to that element. To use filters across all charts in a dashboard, write the chart queries in APL.
</Note>
