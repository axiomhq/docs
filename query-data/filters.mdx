...
### Configure the filter bar for optional filtering

When creating a filter that you want to use with a metrics query, mark its default value as unset:
...
Checking **Unset** tells Axiom to treat the filter as optional. When no option is selected, the variable is not passed to the query and any `ifdef` block that references it is skipped.

### Reference optional filters in MPL queries

Use an `ifdef` block in your MPL query to conditionally apply the filter:
...
Replace `filter_id` with the unique ID you gave the filter, and `tag_name` with the metric tag you want to filter on.

The `ifdef` block runs only when the filter bar has a value selected. When the filter is unset, Axiom skips the block and returns unfiltered data.

**Example:**

The query below filters by `service.name` only when the filter bar with ID `service_filter` has a value selected. When the filter is unset, the query returns data for all services.
...
For more details on `ifdef` syntax, see [Conditional filter](/mpl/introduction#conditional-filter).