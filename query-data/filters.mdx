...
### Define options with an APL query
...
### Define options with a metrics query

To populate the list of options from an MPL metrics query, group by exactly one tag. Axiom uses the distinct values of that tag as the filter options. If the query groups by zero or several tags, Axiom can’t determine the filter options.
