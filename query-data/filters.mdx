...
description: 'Learn how to create dashboards with filters that let you choose the data you want to display.'
sidebarTitle: Filter
tags:
...
Try out all the examples on this page in the [HTTP logs dashboard of the Axiom Playground](https://play.axiom.co/axiom-play-qf1k/dashboards/gZXp8KNJy68q7yGsuA).
...
You can use two types of filters in your dashboards:

- Search filters let you enter text, filter for data that matches the text input, and narrow down the results displayed by the charts in the dashboard. For example, enter **Mac OS**, filter for results that contain this string in the user agent field, and display only the corresponding results in the charts.
- Select filters let you choose one option from a list of options, filter for data that matches the chosen option, and narrow down the results displayed by the charts in the dashboard. For example, choose **France** from the list of countries, filter for results that match the chosen geographical origin, and display only the corresponding results in the charts.

## Use dashboards with filters

To see different filters in action, check out the [HTTP logs dashboard of the Axiom Playground](https://play.axiom.co/axiom-play-qf1k/dashboards/gZXp8KNJy68q7yGsuA). The search filter in the top right lets you search for a specific phrase in the user agent field to display only HTTP requests from a specific user agent. The select filters in the top left let you choose a country and city to display only HTTP requests from a specific geographical origin.

In each chart on your dashboard, you can use all, some, or none of the filters to narrow down the displayed data. For example, in the [HTTP logs dashboard of the Axiom Playground](https://play.axiom.co/axiom-play-qf1k/dashboards/gZXp8KNJy68q7yGsuA), the charts Popular data centers and Popular countries aren't affected by your choices in the select filters. You can use a filter in a chart by [referencing the unique ID of the filter in the chart query](#reference-filters-in-chart-query) as explained later on this page.
...
For each filter, you define a unique ID when you create it. When you create multiple filters, each must have a different ID. You can later use this ID to reference the filter in dashboard charts and other filters.

Filters are visually displayed in your dashboard in a filter bar that you can create and move like any other chart. You can add different types of filters to a single filter bar. A filter bar can contain a maximum of one search filter and any number of select filters.
...
1. Specify a unique filter ID that you'll use to reference the filter. For example, `user_agent_filter`.
...
1. Specify a unique filter ID that you'll use to reference the filter. For example, `country_filter`.
1. In the **Value** section, define the list of options to choose from in the select filter as key-value pairs. Axiom displays the key in the list of options in the filter dropdown and uses the value to filter your data. For example, the key `France` is displayed in the list of options, and the value `FR` is used to filter data in your charts. Define the key-value pairs in one of the following ways:
    ...
    - Choose **Query** to define a dynamic list of options. In this case, Axiom determines the list of options displayed in the filter dynamically based on an APL query. The results of the APL query must contain two fields that Axiom interprets as key-value pairs. Use the `project` command to create key-value fields from any output.
...
The example APL query below uses the distinct values in the `geo.country` field to populate the list of options. It projects these values as both the key and value and sorts them in alphabetical order.
...
Sometimes it makes sense for filters to depend on each other. For example, in one filter you select the country, and in the other filter the city. In this case, the list of options in the city filter depends on your choice in the country filter.

To create a filter that depends on another filter:
...
After creating a filter, specify how you want to use the value chosen in the filter. Include the filter in the APL query of each chart where you want to use the filter to narrow down results. To do this, 
...
The APL query below defines a statistic chart where the displayed data depends on your choice in the filter with the ID `country_filter`. For example, if you choose **France** in the filter, the chart displays only the number of HTTP requests from this geographical origin.
...
You can combine several filters of different types in a chart's query. For example, the APL query below defines a statistic chart where the displayed data depends on three filters:
...
The `declare query_parameters` statement defines the data type of the parameter. For filters, the data type is always string.
