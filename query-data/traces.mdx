...
If you send both OpenTelemetry traces and logs to Axiom, you can correlate them to understand the full context of a request.

### Recommended dataset setup

Send traces and logs to separate datasets. Keeping signal types in separate datasets is a [best practice](/reference/datasets#separate-by-signal-type) because traces and logs have different schemas and query patterns. You can still correlate them using `trace_id`.

### Use span events for contextual logs

The recommended way to attach log-like information to a span is to use [span events](https://opentelemetry.io/docs/concepts/signals/traces/#span-events). Span events are timestamped messages associated with a span and appear in the waterfall view alongside the span they belong to. This is the simplest approach when you want to see contextual information directly within a trace.

### Query logs by trace ID

If you send logs separately (for example, using the OpenTelemetry Logs SDK), correlate them with traces by filtering on the `trace_id` field. For example, to find all logs for a specific trace:
...
### Use span links for asynchronous correlation

For asynchronous operations where a log or event triggers a later trace (or vice versa), use [span links](#span-links) to connect the two. Span links allow you to navigate between related spans across different traces.
