...
OpenTelemetry traces often include custom attributes stored in the `attributes.custom` map field. To filter traces by custom attributes, you must use APL (Axiom Processing Language) queries. The visual query builder does not currently support filtering by custom attributes in map fields.

To filter by custom attributes in APL, use map field syntax to access nested properties:
