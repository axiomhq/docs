...
[Run in Playground](https://play.axiom.co/axiom-play-qf1k/explorer?initForm=%7B%22apl%22%3A%22%5B'otel-demo-traces'%5D%5Cn%7C%20summarize%20count()%20by%20trace_id%22%2C%22queryOptions%22%3A%7B%22quickRange%22%3A%2230m%22%7D%7D)
...
OpenTelemetry defines [Semantic Conventions](https://opentelemetry.io/specs/semconv/) which specify standard attribute names and values for different kinds of operations and data. Attributes that follow semantic conventions will be available as nested fields under the `attributes` field, such as `attributes.http.method`, `attributes.db.system`, etc.
