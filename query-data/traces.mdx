...
Click a trace ID in the results table to show the waterfall view. This view lets you see that span in the context of the entire trace from start to finish.
...
In Axiom, trace events are just like any other events inside datasets. This means they're directly queryable in the UI. While this can be a powerful experience, it's important to note some important details to consider before querying:
...
- For request, rate, and duration aggregations, it's best to include only the root span using `isnull(parent_span_id)`.
...
- You look at a span and you're not familiar with the typical behavior of the service that created it. You want to know if you're looking at something normal in terms of duration or an outlier. The histogram helps you determine if you're looking at an outlier and might need to drill down further.
...
Below are queries that can help get you started with traces inside Axiom. Queries are all executable on the [Axiom Play sandbox](https://axiom.co/play).
...
OpenTelemetry defines [Semantic Conventions](https://opentelemetry.io/docs/specs/semconv/) that specify standard attribute names and values for different kinds of operations and data. Attributes that follow semantic conventions will be available as nested fields under the `attributes` field, such as `attributes.http.method`, `attributes.db.system`, etc.

For example, if a span represents an HTTP request, it may include the following attributes:

- `attributes.http.method`: The HTTP request method (for example, `GET`, `POST`)
...
To create a typed virtual field, you can specify the type, for example:
