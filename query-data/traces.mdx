...
- Attributes that follow semantic conventions are nested fields under the `attributes` map field. For example, `attributes.http.method` or `attributes.http.url`.
- Resource attributes that follow semantic conventions are nested fields under the `resource` map field. For example, `resource.host.name` or `resource.host.id`.
...
Axiom supports OTel semantic conventions from version 1.21 to version 1.32.
...
If you don’t define the version and Axiom can’t detect it, Axiom defaults to version 1.25.0. In this case, Axiom nests attributes that don’t match the semantic conventions of version 1.25.0 under the `attributes.custom` map field.

### Support for new versions

Axiom regularly updates the list of supported versions. If you used an unsupported version that becomes supported after the update, the shape of your data can change:
...
- After the update, Axiom matches these attributes to the newly supported semantic conventions. The newly recognised attributes are nested under the `attributes` map field, similarly to all other attributes that follow semantic conventions.

If the shape of your data changes, update all saved queries, dashboards, and monitors that rely on the newly recognised attributes.
