...
OpenTelemetry traces often include custom attributes stored in the `attributes.custom` map field. To filter traces by custom attributes, use APL (Axiom Processing Language) queries. The visual query builder doesn’t currently support filtering by custom attributes in map fields.

To filter by custom attributes in APL, use map field syntax to access nested properties. For example:
...
For more information on working with map fields and custom attributes, see [Map fields](/apl/data-types/map-fields).
