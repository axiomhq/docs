...
To guarantee good performance, Axioms stops queries that take too much time to generate output and marks these queries as failed. Query timeouts depend on the following:

- **Batched queries** time out after 5 minutes.
- **Progressive queries** do not have a fixed timeout but can fail unpredictably after 90 seconds.
- **Specialized queries**, such as event timeline and trace queries, time out after 59 seconds.

For best performance, optimize queries to complete within these time limits.
