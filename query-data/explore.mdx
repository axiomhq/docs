...
1. Type what you want to infer from your data in your own words using natural language, and then click **Generate**. For example, type `Show me the top status responses in HTTP logs.`
...
After running the query, click <img src="/doc-assets/icons/ellipsis-vertical.svg" className="inline-icon" alt="More icon" /> **More** for additional options:

- **Add to dashboard** converts the query results to a dashboard element and adds it to a dashboard.
- **Create new monitor** uses the query as the basis of a new monitor.
...
- **Copy link with absolute time** copies a link to the query where the time range is fixed to the same time range that see in the query when you create the link. This link shows query results for the same time range, irrespective of when you open the link.
- **Copy as JSON**
