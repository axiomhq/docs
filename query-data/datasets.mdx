...
There are several ways to share queries with your team:

- **Save a query**: When you save a query, it automatically becomes visible to all members of your Axiom organization. Team members can find it in the Saved queries section of the Datasets tab.
- **Copy a link**: In the Query tab, click <Icon icon="ellipsis-vertical" iconType="solid"/> **More** and select **Copy link with relative time** or **Copy link with absolute time** to copy a shareable link. For more information, see [Export query](/query-data/explore#export-query).
- **Share Canvas URL**: In Canvas mode, the URL contains the full state of your queries and charts. Copy and share the URL to let others view the same Canvas. For more information, see [Share Canvas](/query-data/explore#share-canvas).
...
1. Select a dataset.
1. Optional: In the top right of the **Saved queries** section, select whether to display <Icon icon="user" iconType="light"/> your queries or <img src="/doc-assets/icons/many-users.svg" className="inline-icon" alt="Many users icon" /> all queries saved by members of your organization, including queries you saved.
1. Find the query in the list, and then click it to run the query.

Saved queries are automatically visible to all members of your Axiom organization. There's no explicit "share" button—when you save a query, your team members can immediately find and use it.
...
1. Select a dataset.
1. Optional: In the top right of the **Query history** section, select whether to display <Icon icon="user" iconType="light"/> your queries or <img src="/doc-assets/icons/many-users.svg" className="inline-icon" alt="Many users icon" /> your team’s queries.
1. Find the query in the list, and then click it to run the query.