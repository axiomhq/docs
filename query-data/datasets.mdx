...
Sorting on non-projected `_time` and `_sysTime` fields isn’t supported. To sort on `_time` and `_sysTime` fields, project them similarly to the example below:

```apl
['http-logs']
| project my_field, _time
| sort by _time desc
```
