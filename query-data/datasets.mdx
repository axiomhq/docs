...
The event timeline transforms your query results into a detailed distribution, providing a clear visual representation of events over time. It simplifies identifying trends and patterns while also offering a quick way to gauge total events count across a time range.

To get started with the event timeline, go to the Datasets tab or the Explore tab, and run a query on a dataset. When your query results are displayed, the timeline shows the distribution of events across the selected time range. Each bar represents the number of events matched within that specific time bucket. Hovering your cursor over a bar reveals a blue line marking the total events and shows when those events occurred in that particular time range.
