...
Axiom supports secure, centralized user authentication through IdP-initiated flow for SAML-based SSO (identity-provider-initiated flow for Security Assertion Markup Language-based single sign-on). This makes it easy to keep access grants up-to-date with support for the industry standard SCIM protocol. This feature is available for Enterprise customers upon request.
