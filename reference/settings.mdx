...
### Restrict users to specific datasets

By default, a role's dataset-level capabilities apply to all datasets. To restrict a user to only certain datasets, create a role that grants permissions on individual datasets rather than all datasets, and then assign that role to the user.

The example below walks through restricting members of a Security team so they can only query the `security-logs` dataset.

1. Create a custom role for the Security team:
    1. Click <Icon icon="gear" iconType="solid"/> **Settings > Roles**.
    1. Click **New role** and name it, for example, `Security - Read security-logs`.
    1. In the dataset-level capabilities section, find the **Query** row and enable the **Read** permission for the individual dataset `security-logs`. Leave all other datasets without query access.
    1. Assign any organization-level capabilities the role needs, such as read access to dashboards.
1. Create a group for the Security team:
    1. Click <Icon icon="gear" iconType="solid"/> **Settings > Groups**.
    1. Click **New group** and name it, for example, `Security team`.
    1. Assign the `Security - Read security-logs` role to the group.
    1. Add the relevant users to the group.
1. Assign the **None** base role to each restricted user when you invite them or update their role via **Settings > Users**. This ensures their default permissions don't grant access to other datasets. Their access comes entirely from the group membership.

After completing these steps, members of the Security team group can only query the `security-logs` dataset. They can't see or query any other datasets in the organization.

<Tip>
You can apply the same pattern to control ingest access. For example, create a role that grants Ingest (Create) only for a specific dataset and assign it to a group whose members are allowed to send data to that dataset.
</Tip>
...
1. Click <Icon icon="gear" iconType="solid"/> **Settings > Users**.
1. Find the user in the list, and then click <Icon icon="trash" iconType="light"/> **Delete user** on the right.
...
1. Axiom provides you with a setup link to your WorkOS organization.
1. You follow the instructions using the setup link. The setup requires the following attributes for your users:
