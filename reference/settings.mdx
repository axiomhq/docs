...
### Example: Create a dashboard viewer role

To create a role that allows users to view dashboards without admin access:

1. Click <img src="/doc-assets/icons/settings.svg" className="inline-icon" alt="Settings icon" /> **Settings > Roles**.
1. Click **New role**.
1. Enter a name like "Dashboard Viewer" and a description.
1. Assign the following permissions:
   - **Dashboards:** Read only
   - **Datasets:** Read only (or assign dataset-level Query permissions for specific datasets)
   - **Monitors:** Read only (optional, if you want users to see monitor status)
   - **Saved queries:** Read only (optional)
1. Assign this role to users or groups who should have dashboard-only access.

Users with this role can view dashboards shared with them but cannot create, modify, or delete dashboards. The data they see is limited to the datasets they have access to through their role permissions.
