...
#### What happens when you exceed field limits

If you ingest a new event that would exceed the allowed number of fields in a dataset, Axiom returns an error and rejects the event. The error message indicates that the dataset has exceeded its field limit. To prevent this error, ensure that the number of fields in your events are within the allowed limits.

#### How to resolve field limit issues

If you're encountering field limit errors, use one of the following approaches to reduce the number of fields in your dataset:

1. **Vacuum fields**: [Vacuum the dataset fields](/reference/datasets#vacuum-fields) to remove fields that no longer exist in events within your retention period. This rebuilds the schema from current data and can significantly reduce the field count.

2. **Trim old data**: [Trim the dataset](/reference/datasets#trim-dataset) to delete events before a specific date, then vacuum fields. This is useful if you accidentally ingested events with unwanted fields that are still within your retention period.

3. **Use map fields**: [Use map fields](/apl/data-types/map-fields) to consolidate multiple related fields into a single map field. This is particularly useful for high-cardinality data like OpenTelemetry attributes or feature flags.

4. **Upgrade your plan**: If you consistently need more fields, consider upgrading to a plan with higher field limits. Contact Axiom support to discuss your needs.

<Note>
You can check the current number of fields in a dataset by going to **Settings > Datasets and views** and viewing the dataset details.
</Note>
