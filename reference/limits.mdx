...
If you encounter field limit errors, use one of the following approaches to reduce the number of fields in your dataset:

1. [Vacuum the dataset fields](/reference/datasets#vacuum-fields) to remove fields that no longer exist in events within your retention period. This rebuilds the schema from current data and can significantly reduce the field count.

2. [Trim the dataset](/reference/datasets#trim-dataset) to delete events before a specific date, and then [vacuum fields](/reference/datasets#vacuum-fields). This is useful if you accidentally ingested events with unwanted fields that are still within your retention period.

3. [Use map fields](/apl/data-types/map-fields) to consolidate multiple related fields into a single map field. This is particularly useful for high-cardinality data like OpenTelemetry attributes or feature flags.

4. If you consistently need more fields, consider [upgrading your plan](/reference/usage-billing#upgrade-or-downgrade-plan).

<Note>
To check the current number of fields in a dataset, go to **Settings > Datasets and views**, and find the dataset in the list.
</Note>
