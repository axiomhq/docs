...
description: 'Learn how to use the Axiom API to ingest structured data logs and manage your deployments.'
tags: ['axiom documentation', 'documentation', 'axiom', 'API', 'Axiom API', 'Personal Token', 'API Token', 'Labels']
---

Axiom understands your resources and provides an API to ingest structured data logs and manage your deployments. This is a REST-style API that uses JSON for serialization.
...
The personal token can be retrieved from the user's profile page. All users have a personal token. With the personal token, users can access the Axiom API programmatically for custom integrations, management settings, or for tools such as the Axiom CLI.

The personal access token grants access to all resources available to the user on their behalf.

Personal Access tokens can be created under **Profile** or **Settings > Profile > Personal Tokens.**
...
The API token is used to send and query data to one or more datasets. API tokens do not allow control of your organization; they are only used to ingest and query events.
...
To send events directly to the Axiom API, use the [Go client](https://github.com/axiomhq/axiom-go).
...
The following example request contains grouped events. The structure of the `JSON` payload should have the schema of `[ { "labels": { "key1": "value1", "key2": "value12" } }, ]` in which the array comprises one or more JSON objects describing Events.
...
Dataset names are case-sensitive and must be 1 to 128 characters long. They may only contain ASCII alphanumeric characters and the '-' character.
