...
Datasets without a custom retention period inherit your organization’s [default dataset retention period](/reference/organization-settings#set-default-dataset-retention-period). Changing the default therefore also changes the retention period of these datasets, and deletes data in them if you lower the default. Setting a custom retention period on a dataset makes that dataset independent of the default.
...
<Note>
A custom retention period that’s longer than your organization’s default determines how long Axiom stores the data, but not how long you can query it. The default dataset retention period sets the maximum query window for your whole organization. To query data for longer, increase the [default dataset retention period](/reference/organization-settings#set-default-dataset-retention-period).
</Note>
