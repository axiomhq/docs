...
1. In **Edge deployment**, select the edge deployment you want to use for this dataset. You can select from the list of edge deployments you have added to the organization. For more information, see [Edge deployments](/reference/edge-deployments) and [Add edge deployments](/reference/usage-billing#manage-add-ons).
...
1. Click <Icon icon="gear" iconType="solid"/> **Settings > Datasets and views**.
1. In the list, find the dataset where you want to import data, and then click <Icon icon="upload" iconType="regular"/> **Import** on the right.
...
Axiom doesn’t support deleting individual events after ingestion. Axiom stores events in compressed blocks of tens of thousands of events, so granular per-event deletion isn’t possible.

To remove data, use one of these approaches:

- **Trim by date**: Delete all data blocks older than a date you specify. See [Trim dataset](#trim-dataset).
- **Delete by query (exceptional cases)**: Use the [block delete API](/restapi/delete-blocks) to delete all blocks that contain events matching an APL query. This is intended for situations such as accidentally ingesting sensitive data that must be removed immediately. Note that this deletes entire blocks, so it may remove more events than your query matches.
- **Delete the dataset**: Permanently delete all data in the dataset. See [Delete dataset](#delete-dataset).
...
1. Click <Icon icon="gear" iconType="solid"/> **Settings > Datasets and views**.
1. In the list, find the dataset that you want to trim, and then click <Icon icon="scissors" iconType="regular"/> **Trim dataset** on the right.
...
If you hit this limit while prototyping and iterating quickly on your event schema, consider the following alternatives:
- **Use a separate dataset for development.** Keep a dedicated dataset for experimentation so you can delete and recreate it freely without affecting your production data or hitting the vacuum limit on datasets that matter.
- **Delete and recreate the dataset.** If the dataset only contains test data you don't need, delete it and create a fresh one. This resets the field schema without needing to vacuum.
- **Trim before vacuuming.** If the unwanted fields come from events that are still within the retention period, first [trim the dataset](#trim-dataset) to remove those events, then vacuum the fields to rebuild the schema.
...
1. To the right of the sharing link, click <Icon icon="trash" iconType="light"/> **Delete**.
...
1. Click <Icon icon="gear" iconType="solid"/> **Settings > Datasets and views**.
1. In the list, find the dataset, and then click <img src="/doc-assets/icons/share-dataset.svg" className="inline-icon" alt="Share dataset icon" /> **Share dataset** on the right.
1. In the list, find the organization whose access you want to remove, and then click <Icon icon="trash" iconType="light"/> **Remove**.
...
1. Click <Icon icon="gear" iconType="solid"/> **Settings > Datasets and views**.
1. In the list, click the shared dataset that you want to remove, and then click <Icon icon="trash" iconType="light"/> **Remove dataset**.
...
The data retention period determines how long Axiom stores your data. Axiom automatically deletes data that exceeds the retention period. You don't need to manually trim data to stay within the retention window. The automatic deletion runs periodically in the background, so data may remain briefly queryable after it passes the retention threshold.
...
1. Click <Icon icon="gear" iconType="solid"/> **Settings > Datasets and views**.
1. In the list, find the dataset for which you want to change the retention period, and then click <Icon icon="calendar-clock" iconType="sharp-light"/> **Edit dataset retention** on the right.
1. Enter a data retention period. The custom retention period must be greater than 0 days.
1. Click **Submit**.

## Delete events by query

Axiom stores events in blocks of tens of thousands of events. You can't delete individual events after they're ingested, but you can delete all blocks that contain events matching an APL query.

Use the `/v2/datasets/_apl/delete` endpoint to specify an APL query. Axiom deletes every block that contains at least one matching event. This means a deletion request can remove more events than the query matches.

<Warning>
This operation is slow, resource-intensive, and irreversible. Use it only in exceptional cases, such as when you accidentally ingested sensitive data and need to remove it immediately.
</Warning>

For full details and example requests, see [Delete blocks of data](/restapi/delete-blocks).
...
1. Click <Icon icon="gear" iconType="solid"/> **Settings > Datasets and views**.
1. In the list, click the dataset that you want to delete, and then click <Icon icon="trash" iconType="light"/> **Delete dataset**.
...
1. Find the app in the list, and then click <Icon icon="ellipsis-vertical" iconType="solid"/> **More > Edit**.

### Disconnect app

1. Click <Icon icon="gear" iconType="solid"/> **Settings > Apps**.
1. Find the app in the list, and then click <Icon icon="ellipsis-vertical" iconType="solid"/> **More > Disconnect**.
...
1. Click <Icon icon="gear" iconType="solid"/> **Settings > Endpoints**.
1. Find the endpoint in the list, and then click <Icon icon="trash" iconType="light"/> **Delete endpoint** on the right.