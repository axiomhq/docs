...
1. In **Kind**, select one of the following:
    ...
    - Select **OpenTelemetry Logs** if you plan to send OTel log data to the dataset.
    - Select **OpenTelemetry Traces** if you plan to send OTel trace (span) data to the dataset. Traces are stored as events with trace-specific fields such as `trace_id`, `span_id`, and `parent_span_id`.
    - Select **OpenTelemetry Metrics** if you plan to send OTel metric data to the dataset. This option is currently in public preview.
    You must use dedicated datasets for each OTel component. For more information, see [Send OpenTelemetry data to Axiom](/send-data/opentelemetry).
...
You can export query results from the Explorer tab in JSON, CSV, TOON (structured), or TOON (list) format. For more information, see [Export query](/query-data/explore#export-query).

To export data from a dataset:

1. Go to the **Query** tab and run a query against the dataset you want to export. To export all data in a dataset, use a query like `['my-dataset']` with an appropriate time range.
1. Click <Icon icon="ellipsis-vertical" iconType="solid"/> **More** in the top bar.
1. Click **Download results** and select the format you want.

<Note>
Query results are subject to [result size limits](/reference/limits). To export large datasets, break your queries into smaller time ranges and export them separately. Alternatively, use the [Axiom API](/restapi/query) to query data programmatically with pagination.
</Note>
