...
### Understand block-based deletion

Axiom stores data in blocks containing tens of thousands of events. Because trimming operates on entire blocks rather than individual events, a trim operation may remove some events newer than the specified date if they share a block with older data. Conversely, some data older than the specified date may remain if it shares a block with newer data.
