...
## Ensure OpenTelemetry traces aren't split across datasets

In order for the trace viewer and other OpenTelemetry features to work, it's important that all spans for a single trace are in the same dataset. See [Send OpenTelemetry data to Axiom](/send-data/opentelemetry) for more information.

If you want to spans for a specific service, you can filter on the `service.name` field.
...
For example, for a typical distributed application, you could split your data up as follows:

- One dataset with traces for all services
- One dataset with application logs for all services
- One dataset with frontend web vitals
- One dataset with infrastructure logs
- One dataset with security logs
- One dataset with CI logs

If you are looking for some data, you probably know its signal type. But for a distributed system you might not necessarily know in advance which service is responsible for the data you want to query. This approach makes it easy to find the data that you are looking for.
