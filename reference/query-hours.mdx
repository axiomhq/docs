...
Each query is charged at the same rate, regardless of its origin.

Each monitor run counts toward your query costs. For this reason, the frequency (how often the monitor runs) can affect query costs.
...
2. Send a `POST` request to `https://api.axiom.co/v1/datasets/_apl?format=tabular` or `https://api.axiom.co/v1/datasets/_apl?format=legacy` with the following configuration:
