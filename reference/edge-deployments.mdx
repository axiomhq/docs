...
Queries run in a multi-edge organization can only access data from a single edge deployment at a time. When wildcard operators are used to match datasets in queries run in multi-edge organizations, only datasets within the organization’s default edge deployment will be matched.
