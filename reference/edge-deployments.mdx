...
**Important:** All datasets within an organization are stored in the same edge deployment. You cannot have datasets in different regions within the same organization. If you need to store data in a different region, you must create a separate organization for that region. Multi-region support within a single organization is planned for Q1 2026 (see [Multi-region organization](#multi-region-organization)).
