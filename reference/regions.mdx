...
- When you run a query, the query executes in your region, but Axiom routes the query results through the US region. This means that when you query data stored the EU region, the data leaves the region.

The current implementation is useful if you want to ingest data to the EU region, but your data can leave the EU region during querying. If your organization has stricter data sovereignty requirements, Axiom recommends waiting for the next phase of the rollout before migrating.
