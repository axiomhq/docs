...
The following table summarizes which token types work for common use cases:

| Use case | Basic API token | Advanced API token |
| --- | --- | --- |
| Ingest logs | Yes (with dataset access) | Yes (with Ingest permission) |
| Ingest OpenTelemetry traces | No | Yes (with Ingest and Create permissions) |
| Ingest OpenTelemetry metrics | No | Yes (with Ingest and Create permissions) |
| Query data | No | Yes (with Query permission) |
| Manage resources (datasets, monitors) | No | Yes (with relevant permissions) |

<Note>
If you send OpenTelemetry traces or metrics to Axiom using a basic API token, the data may not appear. Use an advanced API token with both Ingest and Create permissions for the target dataset. For more information, see [Send OpenTelemetry data to Axiom](/send-data/opentelemetry).
</Note>
