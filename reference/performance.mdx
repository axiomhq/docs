...
## Practices that limit Axiom performance
...
- **Consider using map fields:** If you have sparse or high-cardinality nested data, consider storing it in a single map (object) field instead of flattening every key. This reduces the total number of top-level fields. Axiom’s [map fields](https://axiom.co/docs/apl/data-types/map-fields#map-fields) are optimized for large objects.
...
- **Ingest as map fields:** Axiom’s new [map column type](https://axiom.co/docs/apl/data-types/map-fields#map-fields) can store object fields column by column, preserving structure and optimizing for nested queries. This allows indexing of specific nested keys.
