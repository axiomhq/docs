...
| [Query memory constraints](#query-memory-constraints) | High | Queries can exceed memory limits and fail with HTTP 432 errors |
...
Some queries can exceed the available memory budget on query workers, causing them to fail with an HTTP 432 error (`StatusQueryMemoryExceeded`). This typically happens when queries use memory-intensive operations on large datasets or wide schemas.
...
When a query exceeds memory limits, Axiom returns an HTTP 432 error with a message indicating memory constraints. The error message specifically mentions memory-intensive operations when detected:

- `parse_json()` on large strings
- `pack(*)` on datasets with many fields
...
- **Extract frequently used fields:** If you must use `parse_json()`, extract only the fields you need rather than parsing entire objects:
...
- **Consider map fields for structured data:** If you frequently need to pack fields, consider restructuring your data to use map fields at ingest time instead.

#### General best practices

- **Reduce query scope:** Use time filters and field-specific filters to reduce the amount of data processed:
...
- **Limit result sets:** Use `limit` to reduce the number of rows returned, especially during exploration:
...
- **Avoid chaining memory-intensive operations:** Multiple memory-intensive operations in sequence can compound memory usage:
    
    Instead of:
    
    ```kusto
    ['logs']
    | extend parsed1 = parse_json(field1)
    | extend parsed2 = parse_json(field2)
    | extend packed = pack(*)
    ```
    
    Restructure to minimize memory usage or split into separate queries.

- **Monitor query costs:** Check the `X-Axiom-Query-Cost-Gbms` header in query responses to identify queries that consume significant resources. For more information, see [Optimize usage](/reference/optimize-usage#determine-query-cost).

For more information on optimizing query performance and usage, see [Optimize usage](/reference/optimize-usage).
