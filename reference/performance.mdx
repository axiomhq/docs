...
- **Inefficient event ordering**: Queries that sort on time are slow.
...
- **Use `search` for substring search:**
    ...
    `search` matches text all fields. To find text in a specific field, a more efficient solution is to use the following:
