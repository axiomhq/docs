...
The Axiom command-line interface (CLI) is a tool that lets you test, manage, and build your Axiom projects by typing commands in the command line.
...
To install the Axiom CLI, make sure you have [Go](https://golang.org/dl/) version 1.16 or higher, then run this command from any directory in your terminal:
...
This command installs Axiom globally so you can run `axiom` commands from any directory.
...
You can check the version and find basic commands for the Axiom CLI by running:
...
The easiest way to start using the Axiom CLI is by logging in through the command line. Run `axiom auth login` or simply `axiom` if no prior configuration exists. This will guide you through a straightforward login process.

## Managing multiple organizations

While most users only need to manage a single Axiom deployment, the Axiom CLI provides the capability to switch between multiple organizations. You can switch between organizations using CLI commands. For example, `axiom auth switch-org` lets you change your active organization, or you can set the `AXIOM_ORG_ID` environment variable for the same purpose.

Every setting in the Axiom CLI can be overwritten using environment variables configured in the `~/.axiom.toml` file. Specifically, `AXIOM_URL`, `AXIOM_TOKEN`, and `AXIOM_ORG_ID` are important for configuring your environment. The `AXIOM_URL` should be set to `https://api.axiom.co`. You can switch between environments using the `axiom auth select` command.
...
One-Click Login is an easier way to authenticate the Axiom CLI and log in to your Axiom deployments and account resources directly in your terminal.
...
You can generate ingest and personal tokens manually in your Axiom user settings.

See [Tokens](/reference/tokens) to learn more about managing access and authorization.

## Configuration and Deployment

The Axiom CLI lets you ingest, authenticate, and stream data.
...
The Axiom CLI supports the ingestion of different data formats (**JSON, NDJSON, and CSV**).
...
Import, transfer, load, and process data for later use or storage using the Axiom CLI. With the [Axiom CLI](https://github.com/axiomhq/cli), you can ingest the contents of a **JSON, NDJSON, or CSV** log file into a dataset.

**To view a list of all available commands, run `axiom` in your terminal:**
...
Below are the commonly used commands in the Axiom CLI:
...
| Commands                    | Description                           |
| --------------------------- | ------------------------------------- |
| **axiom auth login**        | Log in to Axiom                       |
| **axiom auth logout**       | Log out of Axiom                      |
...
To get usage tips and learn more about available commands from within the Axiom CLI, run:

```bash
axiom help
```

For more information about a specific command, run `help` with the command name:
...
**If you have questions or feedback, you can [create an issue](https://github.com/axiomhq/cli/issues) in the Axiom CLI's open-source repository.**

**You can also join our [Discord community](https://axiom.co/discord) to start or join a discussion. We'd love to hear from you!**