...
### Track data ingest by dataset

The [audit log](/reference/audit-log) captures hourly ingestion data for each dataset. Use the following query to identify which datasets contribute most to your ingestion costs:
...
Adjust the threshold for `HourlyIngestGB` based on your expected ingestion patterns. For more information on creating monitors, see [Monitors](/monitor-data/monitors).
...
Use the cost control [Axiom Skill](https://github.com/axiomhq/skills/tree/main/skills/controlling-costs) to automate cost control tasks. The skill helps you:
