...
- **[Builder](/query-data/explore):** A visual interface for building queries.
- **[APL (Axiom Processing Language)](/apl/introduction)** and **[MPL (Metrics Processing Language)](/mpl/introduction)**: Text-based query languages.
...
Start by identifying the correct dataset in the [Datasets page](https://app.axiom.co/datasets). You can explore the fields within a dataset to find relevant information. For example, in `sample-http-logs` you might look at `resp_header_size_bytes` (response size), `status` (HTTP status code), and `geo.country` (geographic origin).

<Tip>
If you're new to writing queries, check out the [AI-assisted query building](#ai-assisted-query-building) section below. You can describe what you want in natural language and Axiom generates the APL for you.
</Tip>

### Tips

- Familiarize yourself with the key datasets your team uses. A quick look at your team's saved queries is a great starting point.
- Pay attention to field data types. A field stored as a number behaves differently than one stored as a string. The schema view in the Axiom console shows the type for each field. For more details, see [Performance best practices](/reference/performance).

<Note>
If a field has mixed types (for example, a status code that is sometimes a number `200` and sometimes a string `"200"`), queries can produce unexpected results. Normalize the data at query time using typecasting functions like `tostring()` or `toint()`. For example, `| where tostring(status) startswith "2"` works reliably on a field with mixed types.
</Note>

## Build your first queries

Axiom provides two main ways to query data:

- **Builder** is a point-and-click interface that helps you build filters and aggregations without writing code. It's excellent for simple, quick-look analyses. For more information, see [Query using Builder](/query-data/explore).
- **APL (Axiom Processing Language)** is a powerful, pipe-based query language for sophisticated analysis, transformation, and aggregation. An APL query starts with a data source (a dataset), followed by operators connected by the pipe `|` character. Each pipe takes the output of the previous line and uses it as input for the next, allowing you to chain operations. A common pattern is dataset, filter (`where`), transform (`extend`), analyze (`summarize`). For more information, see [Introduction to APL](/apl/introduction).

### Example: Count events over time

Count distinct users per day, grouped by HTTP method:
...
- Start with Builder to understand the basic structure of a query, then switch to the APL editor to refine it.
...
- The full list of [tabular operators](/apl/tabular-operators/overview), [aggregation functions](/apl/aggregation-function/statistical-functions), and [scalar functions](/apl/scalar-functions/string-functions) is available in the APL reference.
...
Some datasets are extremely wide, containing hundreds or thousands of fields. When you query these datasets, use `project` to select only the fields you need. Omitting it forces Axiom to retrieve all fields for each event, which dramatically slows down queries.
...
- The query editor does not auto-save. Run your query periodically so it shows in your history, or use the save query feature.
- You can start in Builder and switch to the APL editor to refine. However, you cannot always convert a hand-written APL query back into Builder since not all operations are supported. Once you switch to APL, continue editing in that mode.
...
2. Navigate to the **Monitors** section and create a new Threshold Monitor.
