...
- **[AI-assisted query building](/query-data/query-editor#generate-query-using-natural-language)**: Generate APL queries from natural language descriptions after pressing <kbd>Cmd+K</kbd> (macOS) or <kbd>Ctrl+K</kbd> (Windows/Linux) in the Query tab.
- **[MCP Server](/console/intelligence/mcp-server) and [Skills](/console/intelligence/skills)**: Enable AI agents to query your data.

<Accordion title="Example: APL query that counts distinct users per day">
...
<Accordion title="Example: Alert on increased error rate">
...
1. Paste your query and set the trigger condition, for example, `When error_count is above 50`.
1. Set the schedule, for example, evaluate every 5 minutes.
1. Select or create a [notifier](/monitor-data/notifiers-overview) (Slack, PagerDuty, email, and more).
</Accordion>
...
<Accordion title="Example: Dashboard to monitor HTTP request performance">
