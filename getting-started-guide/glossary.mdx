...
[A](#a) B [C](#c) [D](#d) [E](#e) [F](#f) [G](#g) H [I](#i) K [L](#l) [M](#m) [N](#n) [O](#o) [P](#p) [Q](#q) [R](#r) [S](#s) [T](#t) [U](#u) [V](#v) W X Y Z​
...
In the context of dashboards, annotations are visual elements that add context to the trends displayed in charts and make it easier to investigate issues. For more information, see [Annotate dashboard elements](/query-data/annotate-charts).

In the AI engineering workflow, annotations are expert-provided observations, labels, or corrections added to production traces or evaluation results. Domain experts review AI capability runs and document what went wrong, what should have happened differently, or categorize failure modes. These annotations help identify patterns in capability failures, validate scorer accuracy, and create new test cases for collections. For more information, see [Concepts in AI engineering](/ai-engineering/concepts).
...
An online evaluation is the process of applying a scorer to a capability's live production traffic. This provides real-time feedback on performance degradation, cost, and quality drift, enabling continuous monitoring and improvement of AI capabilities in production.
...
A scorer is a function that evaluates a capability's output. It programmatically assesses quality by comparing the generated output against ground truth or other criteria, returning a score. Scorers are used during evaluations and can also be applied to live production traffic in online evaluations.
