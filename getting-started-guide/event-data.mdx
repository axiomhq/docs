...
Axiom excels at collecting, storing, and analyzing timestamped event data. For logs and traces, Axiom offers unparalleled efficiency and query performance.

For metrics data, Axiom is well-suited for "event-level" metrics that behave more like logs, with each data point representing a discrete event. Your JSON schema fits this model well. Here's a quick example of how your data might look in Axiom:
...
You can easily query and analyze this kind of metrics data in Axiom. For instance, to get average GPU utilization across nodes:
...
Axiom’s metrics support currently comes with the following limitations:
