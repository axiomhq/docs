...
When ingesting data into Axiom, follow these best practices to optimize performance and reliability:

- Batch events: Use a [log forwarder](https://axiom.co/docs/send-data/ingest),[collector](https://axiom.co/docs/send-data/ingest#data-shippers), or [Axiom's official SDKs](https://axiom.co/docs/send-data/ingest#client-libraries) to group multiple events into a single request before sending to Axiom. This reduces the number of API calls and improves overall throughput. Avoid implementing batching within your application itself, as this introduces additional complexity and requires careful management of buffers and error handling.
...
- Handle rate limiting and errors: Implement exponential backoff and retry logic to handle 429 (Too Many Requests) and 503 (Service Unavailable) errors. If ingestion consistently fails after retries, implement a fallback mechanism to store events locally or in cold storage for later processing.
