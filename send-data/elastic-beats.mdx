...
### Monitor Kubernetes events with Metricbeat

Metricbeat can collect metrics and events from Kubernetes clusters, including cluster state, node metrics, pod metrics, and Kubernetes events. This is useful for monitoring cluster health, resource utilization, and troubleshooting issues.

<Steps>
<Step title="Deploy Metricbeat as a DaemonSet">

Deploy Metricbeat as a DaemonSet in your Kubernetes cluster to collect metrics from all nodes. Create a file named `metricbeat-kubernetes.yaml` with the following configuration:
...
spec:
  ...
  template:
    ...
    spec:
      ...
      containers:
        - name: metricbeat
          image: docker.elastic.co/beats/metricbeat-oss:8.12.1
...
<Check>
Verify that Metricbeat pods are running on all nodes:

```bash
kubectl get pods -n kube-system -l app=metricbeat
```
</Check>

</Step>

<Step title="View Kubernetes metrics in Axiom">

After deployment, Metricbeat starts sending Kubernetes metrics and events to Axiom. You can query and visualize:

- Kubernetes events (pod starts, stops, errors)
...
<Tip>
For production deployments, consider adjusting the `period` setting in the configuration to balance between data granularity and ingestion volume. A period of 30s or 60s is often sufficient for most monitoring use cases.
</Tip>
