...
## Forward AWS Application Load Balancer logs and metrics

AWS Application Load Balancer (ALB) logs and metrics are automatically sent to CloudWatch Logs and CloudWatch Metrics. To forward ALB data to Axiom:

### Enable ALB access logs

1. In the AWS Console, navigate to **EC2 > Load Balancers**.
1. Select your Application Load Balancer.
1. Go to the **Monitoring** tab and click **Edit attributes**.
1. Enable **Access logs** and specify an S3 bucket for log storage.
1. ALB access logs will be written to CloudWatch Log Groups with the prefix `/aws/elasticloadbalancing/`.

### Forward ALB logs to Axiom

ALB access logs are stored in CloudWatch Log Groups. Use the Axiom CloudWatch Forwarder to forward these logs:

1. When configuring the Subscriber stack, use a prefix filter with the value `/aws/elasticloadbalancing/` to subscribe to all ALB log groups.
1. Alternatively, use a regular expression filter like `/aws\/elasticloadbalancing\/.*` to match all ALB log groups.

### Forward ALB metrics to Axiom

ALB metrics are published to CloudWatch Metrics. To forward metrics to Axiom:

1. Use the [AWS CloudWatch Metrics Exporter](https://github.com/axiomhq/axiom-cloudwatch-forwarder) or configure a Lambda function to read metrics from CloudWatch and send them to Axiom.
1. Alternatively, use [Fluent Bit](/send-data/fluent-bit) or [Vector](/send-data/vector) with CloudWatch Logs Insights to query and forward metrics data.

For more information about ALB access logs format and CloudWatch metrics, see the [AWS Application Load Balancer documentation](https://docs.aws.amazon.com/elasticloadbalancing/latest/application/load-balancer-access-logs.html).
