...
tags:
  ['logstash', 'axiom documentation', 'documentation', 'axiom', 'guide', 'logs', 'logstash logs', 'grok filter', 'mutate filter', 'drop filter', 'clone filter', 'geoip filter']
logoId: 'logstash'
---

## Logstash

Logstash is an open-source log aggregation and transformation tool and server-side data processing pipeline that simultaneously ingests data from many sources. With Logstash, you can collect, parse, send, and store logs for future use on Axiom.

Logstash works as a data pipeline tool with Axiom, where data is input from your servers and systems on one end, and Axiom processes the data into useful information on the other end.

It can read data from various `input` sources, filter data for the specified configuration, and store it.
...
Specify the `org-id` header if you are using a personal token. However, it's best to use an API token to avoid the need to set the `org-id` header.
...
- [Input stage](https://www.elastic.co/guide/en/logstash/8.0/pipeline.html#_inputs) generates the event and ingests data of all volumes, sizes, forms, and sources
...
For installation instructions for the plugin, see the [OpenSearch documentation](https://opensearch.org/docs/latest/tools/logstash/index/#install-logstash)

In `logstash.conf`, configure your Logstash pipeline to collect and send data logs to Axiom.

The following example shows Logstash configuration that sends data to Axiom:
...
Logstash provides an extensive array of filters that let you enhance, manipulate, and transform your data. These filters can be used to perform tasks such as extracting, removing, and adding new fields and changing the content of fields.

Some valuable filters include the following:

## Grok filter plugin

The Grok filter plugin lets you parse unstructured log data into something structured and queryable, and then send the structured logs to Axiom. It matches the unstructured data to patterns and maps the data to specified fields.
...
The Mutate filter plugin lets you perform general transformations on fields. For example, you can rename, convert, strip, and modify fields in event data.
...
The Drop filter plugin lets you drop certain events based on specified conditions. This helps you filter out unnecessary data.
...
The GeoIP filter plugin adds information about the geographical location of IP addresses. This data includes the latitude, longitude, continent, country, and other location details.
...
This configuration adds geographical location data for the IP address in the `ip` field. Note that you might need to specify the path to the GeoIP database file in the plugin configuration, depending on your setup.