...
- **Message size limit:** Axiom currently enforces an 8KB per-message size limit. This is in line with RFC5425 guidelines. Any message exceeding the limit causes the connection to close because Axiom doesn’t support ingesting truncated messages.
...
Ensure your messages conform to the size limit and TLS requirements. Frequent connection re-establishment may indicate that messages are rejected due to size or other formatting issues.
