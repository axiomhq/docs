...
Syslog is an outdated protocol from the 1980s. Some of its limitations include:

- Lack of error reporting and feedback mechanisms when issues occur
- Inability to gracefully terminate connections, which can result in missing data
...
- **Message size limit:** Axiom currently enforces a 64KB per-message size limit. This aligns with RFC5425 guidelines. Any message exceeding the limit causes the connection to close because Axiom doesn't support ingesting truncated messages.
...
Ensure your messages conform to the size limit and TLS requirements. If the connection is frequently reestablished and messages are rejected, the issue might be related to message size or other formatting issues.

## Install Axiom Syslog Proxy

To install the Axiom Syslog Proxy, choose one of the following options:
- [Install using a precompiled binary file](#install-using-pre-compiled-binary-file)
...
### Install using precompiled binary file

To install the Axiom Syslog Proxy using a precompiled binary file, download one of the [releases on GitHub](https://github.com/axiomhq/axiom-syslog-proxy/releases/latest).

### Install using Homebrew

Run the following command to install the Axiom Syslog Proxy using Homebrew:
...
Run the following command to install the Axiom Syslog Proxy using `go get`:
...
Run the following commands to install the Axiom Syslog Proxy from the GitHub source:
...
To run Axiom Syslog Proxy, run the following command in your terminal:
...
To test the Axiom Syslog Proxy configuration:

1. Run the following commands in your terminal to send two messages:
...
1. Check that Axiom displays the messages you sent.