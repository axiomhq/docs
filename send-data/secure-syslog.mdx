...
- **TLS requirement:** Axiom only supports syslog over TLS, specifically following RFC5425. Configure your syslog client with a self-generated certificate or a custom server certificate. For more information, see the [RFC Series documentation](https://www.rfc-editor.org/rfc/rfc5425).
- **Port requirements:** TCP log messages are sent on TCP port `601`.
...
Configure your syslog client to connect to Axiom with the following:
- The target URL for the endpoint you have generated in Axiom by following the procedure above. For example, `https://opbizplsf8klnw.ingress.axiom.co`.
