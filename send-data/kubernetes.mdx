...
data:
  filebeat.yml: |-
    ...
    output.elasticsearch:
      hosts: ['${AXIOM_HOST}/v1/datasets/${{dataset_name}}/elastic']
      api_key: 'axiom:${AXIOM_{api_token}}'
...
- `{dataset_name}` is your [dataset](/reference/datasets) name.
- `AXIOM_{api_token}` is your Axiom API token. To create an API key, see [Access settings](/reference/settings#access-overview).
...
data:
  fluent-bit.conf: |-
    ...
    [OUTPUT]
        ...
        Header          Authorization Bearer ${AXIOM_{api_token}}
        Format          json
        Json_date_key   time
