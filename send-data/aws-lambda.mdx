...
Use Axiom Lambda Extension to send logs and platform events of your Lambda function to Axiom. Axiom detects the extension and provides you with quick filters and a dashboard.
...
1. [Turn off the permissions for CloudWatch](#turn-off-cloudwatch-logging).

The last step is important because after you install the Axiom Lambda extension, the Lambda service still sends logs to CloudWatch Logs. You need to manually turn off CloudWatch logging.
...
- Use plain Terraform code. For example:
...
- Use the [AWS Lambda Terraform module](https://registry.terraform.io/modules/terraform-aws-modules/lambda/aws/latest). For example:
...
First, add a layer to your Lambda function:
...
Second, add the Axiom dataset name and API token to the list of environment variables:
...
## Turn off CloudWatch logging

After you install the Axiom Lambda extension, the Lambda service still sends logs to CloudWatch Logs. You need to manually turn off CloudWatch logging.

To turn off CloudWatch logging, deny the Lambda function access to CloudWatch by editing the permissions:

1. In the AWS Lambda function UI, go to **Configuration > Permissions**.
1. In the **Execution role** section, click the role related to CloudWatch Logs.
1. In the **Permissions** tab, select the role and click **Remove** to deny access to CloudWatch Logs.
