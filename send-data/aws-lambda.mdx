...
## Best Practices using AWS Lambda

When using AWS Lambda for ingestion, consider these additional recommendations:

- Axiom Lambda extension: Use the [Axiom Lambda extension](/docs/send-data/aws-lambda) for efficient log collection directly from your Lambda functions.

- Minimize direct API calls: Avoid making individual API calls for each Lambda invocation. Instead, use the [Lambda extension](/docs/send-data/aws-lambda) or aggregate logs before sending.

- Terminate early: In Lambda functions, prioritize quick execution and termination. Avoid waiting for ingestion confirmation within the function itself.

- Alternative routing option: If you aren‘t using the Axiom Lambda layer, consider routing to an internal gateway that handles batching and processing. For example: Lambda -> CloudWatch Logs (short retention) -> [Log Forwarder](https://github.com/axiomhq/axiom-cloudwatch-forwarder). This approach allows for efficient log aggregation and processing before sending to Axiom.