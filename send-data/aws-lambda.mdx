...
Axiom detects the extension and provides you with quick filters and a dashboard. For more information about how this enriches your Axiom organization, see [AWS Lambda app](/apps/lambda).
...
The last step is important because after you install the Axiom Lambda extension, the Lambda service continues to send logs to Amazon CloudWatch Logs. You must manually turn off Amazon CloudWatch logging.
...
Add the Axiom dataset name and API token to the list of environment variables. For more information about setting environment variables, see the [AWS documentation](https://docs.aws.amazon.com/lambda/latest/dg/configuration-envvars.html).
...
Add a new layer to your Lambda function with the following ARN (Amazon Resource Name). For more information about adding layers to your function, see the [AWS documentation](https://docs.aws.amazon.com/lambda/latest/dg/adding-layers.html).
```bash
arn:aws:lambda:AWS_REGION:694952825951:layer:axiom-extension-ARCH:VERSION
```
...
After you install the Axiom Lambda extension, the Lambda service continues to send logs to CloudWatch Logs. You must manually turn off Amazon CloudWatch logging.
...
For testing purposes, set the `PANIC_ON_API_ERR` environment variable to `true`. This causes the Axiom Lambda Extension to crash if it can't connect to Axiom.