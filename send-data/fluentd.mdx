...
With Fluentd, you can collect logs from multiple sources and ship them instantly into Axiom.

## Installation

Visit the [Fluentd download page](https://www.fluentd.org/download) to install Fluentd on your system.

You need to specify the org-id header if you are using a personal token. It's best to use an API token to avoid the need to specify the org-id header.

Learn more about [API and personal tokens](/reference/tokens).

## Configuration

The Fluentd lifecycle consists of five different components:
...
- Labels: Group filters and simplify tag handling
