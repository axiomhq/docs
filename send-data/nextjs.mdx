...
5.  In the `lib/axiom` folder, create a `client.ts` file with the following content:
    ...
    <Warning>
    Make sure the token you are using on the client side has the appropriate permissions, we recommend using only ingest permissions into a specific dataset for the client side.
    If you don't want to expose the token to the client, you can use the a [proxy transport](#proxy-for-client-side-usage) to send logs to Axiom.
    </Warning>
...
Instead of sending logs directly to Axiom, you can send them to a proxy endpoint in your Next.js app. This is useful if you don't want to expose the Axiom token to the client, or if you want to send the logs from the client to transports on your server.
...
2. In the `/app/api/axiom` folder, create a `route.ts` file with the following content. In this example, we are using `/api/axiom` as the Axiom proxy path.
