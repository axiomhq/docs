...
<Warning>
Axiom datasets have a limit on the number of fields they can store. Exceeding this limit causes ingest to fail for the affected events. To avoid hitting the limit, use a fixed, predictable set of field keys in the `fields` object. Avoid spreading arbitrary objects or using dynamic keys, such as user-supplied input or per-request identifiers, as top-level fields. If you need to store unpredictable data, nest it inside a single field as a string or use [map fields](/apl/data-types/map-fields). For more information, see [Limits](/reference/limits).
</Warning>
