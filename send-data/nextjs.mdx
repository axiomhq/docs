...
- [axiom-nextjs library](#use-axiom-nextjs-library) (public preview)

<Note>
    The axiom-nextjs library is currently in public preview. For more information, see [Features states](/getting-started-guide/feature-states).
</Note>
...
- The two options can collect different event types.
    | Event type | Axiom Vercel app | next-axiom library | axiom-nextjs library |
    | --- | --- | --- | --- |
...
The rest of this page explains how to send data from your Next.js app to Axiom using the next-axiom or the axiom-nextjs library.
...
1. Add the following environment variables to your Next.js app:
    ...
    - `NEXT_PUBLIC_AXIOM_PROXY_PATH` is the Axiom proxy path where the client stores logs. This example uses the path `/api/axiom`.
...
1. In the `lib/axiom` folder, create a `server.ts` file with the following content:
    ...
    The `createAxiomRouteHandler` is a builder function that returns a wrapper for your route handlers. The wrapper handles successful responses and errors thrown within the route handler. For more information on the logger, see [Axiom Logger](/send-data/javascript-logging).
...
1. In the `/app/api/axiom/` folder, create a `route.ts` file with the following content. In this example, `/app/api/axiom/` is the Axiom proxy path specified above.
...
Some common customizations of route handlers:
