...
- [@axiomhq/nextjs library](#use-axiomhqnextjs-library)
...
1. In the `lib/axiom` folder, create a `server.ts` file with the following content:
    ...
    The `createAxiomRouteHandler` is a builder function that returns a wrapper for your route handlers. The wrapper handles successful responses and errors thrown within the route handler. For more information on the logger, see [the @axiomhq/logging library](/send-data/javascript#use-axiomhqlogging).
