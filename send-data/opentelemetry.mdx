...
Axiom doesn't apply sampling to incoming data. If you want to reduce the volume of traces or logs before they reach Axiom, configure sampling in your OpenTelemetry Collector using processors from the [opentelemetry-collector-contrib](https://github.com/open-telemetry/opentelemetry-collector-contrib) distribution.

### Probabilistic sampling

The `probabilistic_sampler` processor randomly drops a configurable percentage of spans or log records. Use it when you want to uniformly reduce volume across all services.

```yaml
processors:
  probabilistic_sampler:
    sampling_percentage: 20  # Keep 20% of spans
...
The `tail_sampling` processor makes sampling decisions after collecting all spans in a trace. This lets you apply policy-based rules, such as always keeping error traces while dropping a percentage of successful ones.
...
Tail sampling requires all spans from the same trace to reach the same Collector instance. If you run multiple Collector instances, add a load-balancing exporter in front of the tail-sampling layer.
