...
<Note>
To send OpenTelemetry traces or metrics to Axiom, use an advanced API token with both Ingest and Create permissions for the target dataset. Basic API tokens are not sufficient for trace and metric ingestion. For more information, see [Choose the right token type](/reference/tokens#choose-the-right-token-type).

When you create a dataset for OTel data, select the appropriate dataset kind: **OpenTelemetry Traces**, **OpenTelemetry Logs**, or **OpenTelemetry Metrics**. For more information, see [Create dataset](/reference/datasets#create-dataset).
</Note>
