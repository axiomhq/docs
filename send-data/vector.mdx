...
Vector is a lightweight and ultra-fast tool for building observability pipelines. It has a built-in support for shipping logs to Axiom through the [`axiom` sink](https://vector.dev/reference/configuration/sinks/axiom/).

## Installation

Follow the [quickstart guide in the Vector documentation](https://vector.dev/setup/quickstart/) to install Vector, and to configure sources and sinks.
...
Send data to Axiom with Vector using the [`file` method](https://vector.dev/reference/configuration/sources/file/) and the [`axiom` sink](https://vector.dev/reference/configuration/sinks/axiom/).
...
<Note>
Any changes to Vector’s `file` method can make the code example above outdated. If this happens, please refer to the [official Vector documentation on the `file` method](https://vector.dev/reference/configuration/sources/file/), and we kindly ask you to inform us of the issue using the feedback tool at the bottom of this page.
</Note>
...
Check out the [advanced configuration on Batch, Buffer configuration, and Encoding on Vector Documentation](https://vector.dev/reference/configuration/sinks/axiom/)