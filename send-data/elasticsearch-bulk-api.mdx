...
<Note>
Axiom determines the target dataset from the URL path (`/v1/datasets/DATASET_NAME/elastic/_bulk`), not from the `_index` field in the request body. While the `_index` field is included in the examples above for compatibility with the Elasticsearch Bulk API format, Axiom ignores this value. Always specify the target dataset in the URL path.
</Note>
