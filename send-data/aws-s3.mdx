...
sidebarTitle: AWS S3
tags: ['aws', 'documentation', 'axiom documentation', 'AWS', 's3', 'lambda']
logoId: 'awsbucket'
---
...
Ensure the log file you upload to the S3 bucket is in the correct format, such as JSON, newline-delimited JSON (NDJSON), or CSV. Here's an example:
...
After uploading a test log file to your S3 bucket, the Lambda function automatically processes the log data and sends it to Axiom. In Axiom, go to the Datasets tab and select the dataset you specified in the Lambda function. You should now see your logs from your IoT devices in Axiom.