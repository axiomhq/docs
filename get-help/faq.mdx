...
2. Stateless ingest pipeline that requires minimal compute/memory to store as much as 1.5TB/day per vCPU

3. Ingests all data into object storage, enabling the cheapest storage possible for all ingested data

4. Enables querying scale-out with cloud functions, requiring no constantly-running servers waiting for a query to be processed. Instead, enjoy zero-to-infinite querying instantly
...
5. Free your organization to do more with its data
...
Retention on Axiom's [Enterprise plan](https://axiom.co/pricing) can be customized to your needs, with the option for unlimited retention so your organization has access to all its data, all the time.
