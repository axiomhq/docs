...
Splunk uses a `search` command for basic searching, while in APL, you simply specify the dataset name followed by a filter.
...
In Splunk, rename fields using the `rename` command. In APL, rename fields using the `extend` and `project` operators. Here is the general syntax:
...
In Splunk, use the `eval` command to create calculated fields based on the values of other fields. In APL, use the `extend` operator to create calculated fields based on the values of other fields.
...
| Query and search                 | search      | query                           | Concepts are essentially the same between APL and Splunk| 
...
In Splunk, the function is invoked by using the `eval` operator. In APL, it's used as part of the `extend` or `project` operators.
...
|       APL                   | limit             | ['sample-http-logs'] <br />&#124; limit 100 |
...
| Product | Operator | Example |
|:---|:---|:---|
| Splunk | rename |  Sample.Logs=330009.2<br />&#124; rename Date.Exception as exception |
...
| Product | Operator | Example |
|:---|:---|:---|
| Splunk | **fields -** | Sample.Logs=330009.2<br />&#124; fields - quota, highest_seller |
...
Dive into the Axiom Processing Language, start converting your Splunk queries to APL, and explore the rich capabilities of the Query tab. Embrace the learning curve, and remember that every complex query you master is another step forward in your data analytics journey.