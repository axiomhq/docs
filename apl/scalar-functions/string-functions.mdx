...
| [redact()](#redact)                           | Replace values in a field based on the specified regular expression. |
...
Replace fields based on the specified regular expression.
...
we are rolling out our redact operator which will allow to replace fields based on a specified regular expression. This enhances our notion to value each data event equally and make Axiom the single plane of truth for an organization’s data lifecycle. As such the redact features is part of a family of features including Flow and our RBAC with other features coming soon out to enable user to create data products and have better ownership and control over their data.