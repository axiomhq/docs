...
- [strlen](/apl/string-functions/strlen): Returns the number of characters in a string. Use `strlen` if you are only working with strings and want stricter typing.
- [array_length](/apl/array-functions/array_length): Returns the number of elements in an array. Use this when working specifically with arrays.
- [isnull](/apl/scalar-functions/isnull): Checks if a value is null. Combine with `len` when you want to filter out null values before computing length.
- [isempty](/apl/scalar-functions/isempty): Checks if a string or array is empty. Use this when you want a true/false check rather than a numeric length.
- [tostring](/apl/conversion-functions/tostring): Converts input to string. Use it to coerce values before applying `len`.
