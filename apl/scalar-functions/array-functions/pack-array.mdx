...
You can use `pack_array(*)` to pack all fields from the current row into an array. Note that this packs only the values, not the field names. If you need key-value pairs, use `bag_pack(*)` instead.
