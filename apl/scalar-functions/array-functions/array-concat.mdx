...
['sample-http-logs']
| take 50
| summarize combined_requests = array_concat(make_list(uri), make_list(method))
```

[Run in Playground](https://play.axiom.co/axiom-play-qf1k/explorer?initForm=%7B%22apl%22%3A%22%5B'sample-http-logs'%5D%20%7C%20take%2050%20%7C%20summarize%20combined_requests%20%3D%20array_concat(make_list(uri)%2C%20make_list(method))%22%7D)
...
['otel-demo-traces']
| take 50
| summarize combined_ids = array_concat(make_list(span_id), make_list(trace_id))
```

[Run in Playground](https://play.axiom.co/axiom-play-qf1k/explorer?initForm=%7B%22apl%22%3A%22%5B'otel-demo-traces'%5D%20%7C%20take%2050%20%7C%20summarize%20combined_ids%20%3D%20array_concat(make_list(span_id)%2C%20make_list(trace_id))%22%7D)
...
| where status == '401'
| take 50
| summarize failed_attempts = array_concat(make_list(id), make_list(['geo.city']))
```

[Run in Playground](https://play.axiom.co/axiom-play-qf1k/explorer?initForm=%7B%22apl%22%3A%22%5B'sample-http-logs'%5D%20%7C%20where%20status%20%3D%3D%20'401'%20%7C%20take%2050%20%7C%20summarize%20failed_attempts%20%3D%20array_concat(make_list(id)%2C%20make_list(%5B'geo.city'%5D))%22%7D)
