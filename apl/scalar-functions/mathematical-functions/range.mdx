...
- [make_series](/apl/aggregation-functions/make_series): Creates a time series by aggregating values into buckets. Use `make_series` when working with grouped data over time intervals.
- [extend](/apl/query-operators/extend): Adds new computed columns to a dataset. Use with `range` to derive additional values from a generated sequence.
- [top](/apl/aggregation-functions/top): Returns the top N rows by some expression. Use it after `range` if you want only the highest N values from a generated sequence.
- [summarize](/apl/query-operators/summarize): Aggregates data over groups. Combine with `range` to group generated intervals.
- [join](/apl/query-operators/join): Merges rows from two tables. Use with `range` to match synthetic data against real datasets.
