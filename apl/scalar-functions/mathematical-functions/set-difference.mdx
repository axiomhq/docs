...
* [set_intersect](/apl/array-functions/set_intersect): Returns elements common to both arrays. Use when you need shared values instead of differences.
* [set_union](/apl/array-functions/set_union): Combines all unique elements from both arrays. Use when you want the full set of distinct values.
* [array_length](/apl/array-functions/array_length): Returns the number of elements in an array. Useful for counting after applying `set_difference`.
* [make_set](/apl/aggregation-functions/make_set): Creates a set of distinct values during aggregation. Use this before computing differences.
* [array_concat](/apl/array-functions/array_concat): Merges two arrays. Use when you want to join arrays without removing duplicates.
