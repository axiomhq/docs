...
| ------------------------------------------------ | ----------------------------------------------------------------------------------------- |
| [ensure_field()](#ensure-field)              | Ensures the existence of a field and returns its value or a typed null if it doesn't exist. |
...
| [isbool()](#isbool)                          | Returns true or false based on whether the expression value is boolean.                   |
| [toint()](#toint)                          | Converts the input to an integer value (signed 64-bit) number representation.                |

## ensure_field()

Ensures the existence of a field and returns its value or a typed null if it doesn't exist.
...
This function returns the value of the specified field if it exists; otherwise, it returns a typed null.
...
In this example, the value of `show_field` is null because the `myfield` field doesn't exist.
...
If the conversion is successful, the result will be a datetime value. Otherwise, the result will be `false`.
...
Converts the input to a value of type real. **(todouble() is an alternative to toreal())**
...
If conversion is successful, the result is a value of type real. If conversion is not successful, the result is false.
...
If conversion is successful, result will be a timespan value. Otherwise, result will be false.
...
- If the input is a scalar value of type other than `dynamic`, the output is the result of `tostring()` applied to that value.
...
Returns true or false based on whether the expression value is boolean.

### Arguments

- Expr: The function accepts one argument. The variable or expression to be evaluated.
