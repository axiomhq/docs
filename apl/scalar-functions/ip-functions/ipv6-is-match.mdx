...
\[Run in Playground]\([https://play.axiom.co/axiom-play-qf1k/query?initForm=%7B%22apl%22%3A%22%5B'sample-http-logs'%5D](https://play.axiom.co/axiom-play-qf1k/query?initForm=%7B%22apl%22%3A%22%5B'sample-http-logs'%5D) %7C extend is\_internal%20%3D%20ipv6\_is\_match(id%2C%20'2001%3Adb8%3A%3A%2F32') %7C where is\_internal %7C project \_time%2C id%2C uri%2C method%2C status%22%7D)
...
\[Run in Playground]\([https://play.axiom.co/axiom-play-qf1k/query?initForm=%7B%22apl%22%3A%22%5B'otel-demo-traces'%5D](https://play.axiom.co/axiom-play-qf1k/query?initForm=%7B%22apl%22%3A%22%5B'otel-demo-traces'%5D) %7C extend is\_mesh%20%3D%20ipv6\_is\_match(span\_id%2C%20'2001%3Adb8%3Aac10%3A%3A%2F40') %7C where is\_mesh %7C project \_time%2C span\_id%2C trace\_id%2C%20%5B'service.name'%5D%2C kind%2C status\_code%22%7D)

**Output**

| \_time               | span\_id              | trace\_id | service.name    | kind   | status\_code |
| -------------------- | --------------------- | --------- | --------------- | ------ | ------------ |
...
\[Run in Playground]\([https://play.axiom.co/axiom-play-qf1k/query?initForm=%7B%22apl%22%3A%22%5B'sample-http-logs'%5D](https://play.axiom.co/axiom-play-qf1k/query?initForm=%7B%22apl%22%3A%22%5B'sample-http-logs'%5D) %7C extend is\_suspicious%20%3D%20ipv6\_is\_match(id%2C%20'fd00%3A1337%3A%3A%2F48') %7C where is\_suspicious %7C summarize count()%20by%20uri%2C%20status%2C%20%5B'geo.country'%5D%22%7D)
...
* [ipv4\_is\_match](/apl/ip-functions/ipv4_is_match): Checks whether an IPv4 address belongs to a specified IPv4 subnet. Use it when working with IPv4 addresses.
* [parse\_ipv6](/apl/ip-functions/parse_ipv6): Parses a string into an IPv6 address. Use it when you need to validate or normalize IPv6 input before matching.
* [parse\_ipv4](/apl/ip-functions/parse_ipv4): Parses a string into an IPv4 address. Use it when working with raw IPv4 strings.
* [is\_ipv4](/apl/ip-functions/is_ipv4): Returns `true` if the input is a valid IPv4 address. Use it to distinguish address formats before applying a match.
* [is\_ipv6](/apl/ip-functions/is_ipv6): Returns `true` if the input is a valid IPv6 address. Use it to filter out invalid or non-IPv6 data.
