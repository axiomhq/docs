...
- [ipv4_is_match](/apl/ip-functions/ipv4_is_match): Checks whether an IPv4 address belongs to a specified IPv4 subnet. Use it when working with IPv4 addresses.
- [parse_ipv6](/apl/ip-functions/parse_ipv6): Parses a string into an IPv6 address. Use it when you need to validate or normalize IPv6 input before matching.
- [parse_ipv4](/apl/ip-functions/parse_ipv4): Parses a string into an IPv4 address. Use it when working with raw IPv4 strings.
- [is_ipv4](/apl/ip-functions/is_ipv4): Returns `true` if the input is a valid IPv4 address. Use it to distinguish address formats before applying a match.
- [is_ipv6](/apl/ip-functions/is_ipv6): Returns `true` if the input is a valid IPv6 address. Use it to filter out invalid or non-IPv6 data.
