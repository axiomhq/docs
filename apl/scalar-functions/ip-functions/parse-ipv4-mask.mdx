...
- [**split**](/apl/string-functions/split-function): Use for splitting strings if you don’t need the IP-specific parsing of `parse_ipv4_mask`.
- [**extract_ipv4**](/apl/network-functions/extract_ipv4-function): Use for extracting IPv4 addresses without subnet mask parsing.
- [**cidr_match**](/apl/network-functions/cidr_match-function): Use for checking whether an IP falls within a specific CIDR range.