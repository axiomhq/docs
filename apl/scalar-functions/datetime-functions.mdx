...
```kusto
['sample-http-logs']
| summarize earliest = min(_time)
| extend earliest_str = tostring(earliest)
```
...
['sample-http-logs']
| summarize earliest = min(_time)
| extend date_only = tostring(startofday(earliest))
```

To extract individual components such as year, month, or day, use [datetime_part](/apl/scalar-functions/datetime-functions/datetime-part), [getyear](/apl/scalar-functions/datetime-functions/getyear), [getmonth](/apl/scalar-functions/datetime-functions/getmonth), or [dayofmonth](/apl/scalar-functions/datetime-functions/dayofmonth).
