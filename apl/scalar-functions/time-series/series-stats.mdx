...
| project id, 
    min_duration = statistics[0],
    max_duration = statistics[2],
    avg_duration = statistics[4],
    stdev_duration = statistics[6]
| take 5
```

[Run in Playground](https://play.axiom.co/axiom-play-qf1k/query?initForm=%7B%22apl%22%3A%22%5B'sample-http-logs'%5D%20%7C%20summarize%20durations%20%3D%20make_list(req_duration_ms)%20by%20id%20%7C%20extend%20statistics%20%3D%20series_stats(durations)%20%7C%20project%20id%2C%20min_duration%20%3D%20statistics%5B0%5D%2C%20max_duration%20%3D%20statistics%5B2%5D%2C%20avg_duration%20%3D%20statistics%5B4%5D%2C%20stdev_duration%20%3D%20statistics%5B6%5D%20%7C%20take%205%22%7D)
...
This query calculates comprehensive statistics for each user's request durations by extracting specific elements from the 7-element statistics array.

</Tab>
<Tab title="OpenTelemetry traces">

In OpenTelemetry traces, you can use `series_stats` to analyze HTTP status code distribution, including where the minimum and maximum values occur in the series.
...
| extend statistics = series_stats(status_codes)
| project ['service.name'],
    min_status = statistics[0],
    min_position = statistics[1],
    max_status = statistics[2],
    max_position = statistics[3],
    avg_status = statistics[4]
| take 5
```

[Run in Playground](https://play.axiom.co/axiom-play-qf1k/query?initForm=%7B%22apl%22%3A%22%5B'otel-demo-traces'%5D%20%7C%20where%20isnotnull(%5B'attributes.http.status_code'%5D)%20%7C%20summarize%20status_codes%20%3D%20make_list(tolong(%5B'attributes.http.status_code'%5D))%20by%20%5B'service.name'%5D%20%7C%20extend%20statistics%20%3D%20series_stats(status_codes)%20%7C%20project%20%5B'service.name'%5D%2C%20min_status%20%3D%20statistics%5B0%5D%2C%20min_position%20%3D%20statistics%5B1%5D%2C%20max_status%20%3D%20statistics%5B2%5D%2C%20max_position%20%3D%20statistics%5B3%5D%2C%20avg_status%20%3D%20statistics%5B4%5D%20%7C%20take%205%22%7D)

**Output**

| service.name | min_status | min_position | max_status | max_position | avg_status |
| ------------ | ---------- | ------------ | ---------- | ------------ | ---------- |
| frontend     | 200        | 0            | 500        | 42           | 215.3      |
| checkout     | 200        | 0            | 404        | 18           | 202.1      |

This query provides statistical analysis of HTTP status codes per service, including the positions where min and max values occur.
...
| extend statistics = series_stats(durations)
| project status,
    typical_duration = statistics[4],
    variance = statistics[5],
    stdev = statistics[6],
    max_observed = statistics[2]
| extend anomaly_threshold = typical_duration + (3 * stdev)
```

[Run in Playground](https://play.axiom.co/axiom-play-qf1k/query?initForm=%7B%22apl%22%3A%22%5B'sample-http-logs'%5D%20%7C%20summarize%20durations%20%3D%20make_list(req_duration_ms)%20by%20status%20%7C%20extend%20statistics%20%3D%20series_stats(durations)%20%7C%20project%20status%2C%20typical_duration%20%3D%20statistics%5B4%5D%2C%20variance%20%3D%20statistics%5B5%5D%2C%20stdev%20%3D%20statistics%5B6%5D%2C%20max_observed%20%3D%20statistics%5B2%5D%20%7C%20extend%20anomaly_threshold%20%3D%20typical_duration%20%2B%20(3%20*%20stdev)%22%7D)
