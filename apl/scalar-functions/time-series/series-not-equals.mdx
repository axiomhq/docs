...
[Run in Playground]([https://play.axiom.co/axiom-play-qf1k/query?initForm=%7B%22apl%22%3A%22%5B'sample-http-logs'%5D](https://play.axiom.co/axiom-play-qf1k/query?initForm=%7B%22apl%22%3A%22%5B'sample-http-logs'%5D) %7C summarize durations1%20%3D%20make_list(req_duration_ms)%20by%20method %7C join%20(%20%20%20%5B'sample-http-logs'%5D %20%20%20%7C%20summarize%20durations2%20%3D%20make_list(req_duration_ms)%20by%20method )%20on%20method %7C extend%20diff_flags%20%3D%20series_not_equals(durations1%2C%20durations2)%22%7D)
...
[Run in Playground]([https://play.axiom.co/axiom-play-qf1k/query?initForm=%7B%22apl%22%3A%22%5B'otel-demo-traces'%5D](https://play.axiom.co/axiom-play-qf1k/query?initForm=%7B%22apl%22%3A%22%5B'otel-demo-traces'%5D) %7C where%20service.name%20%3D%3D%20'frontend' %7C summarize%20frontend_durations%20%3D%20make_list(duration)%20by%20trace_id %7C join%20(%20%20%20%5B'otel-demo-traces'%5D %20%20%20%7C%20where%20service.name%20%3D%3D%20'checkoutservice' %20%20%20%7C%20summarize%20checkout_durations%20%3D%20make_list(duration)%20by%20trace_id )%20on%20trace_id %7C extend%20diff_flags%20%3D%20series_not_equals(frontend_durations%2C%20checkout_durations)%22%7D)
...
[Run in Playground]([https://play.axiom.co/axiom-play-qf1k/query?initForm=%7B%22apl%22%3A%22%5B'sample-http-logs'%5D](https://play.axiom.co/axiom-play-qf1k/query?initForm=%7B%22apl%22%3A%22%5B'sample-http-logs'%5D) %7C where%20geo.country%20%3D%3D%20'US' %7C summarize%20us_statuses%20%3D%20make_list(status)%20by%20uri %7C join%20(%20%20%20%5B'sample-http-logs'%5D %20%20%20%7C%20where%20geo.country%20%3D%3D%20'DE' %20%20%20%7C%20summarize%20de_statuses%20%3D%20make_list(status)%20by%20uri )%20on%20uri %7C extend%20diff_flags%20%3D%20series_not_equals(us_statuses%2C%20de_statuses)%22%7D)
...
* [series_equals](/apl/series-functions/series_equals): Returns a Boolean array showing where two arrays are equal element by element. Use this when you want to find matches instead of mismatches.
* [series_greater](/apl/series-functions/series_greater): Returns a Boolean array showing where elements in the first array are greater than the second. Use this to compare performance metrics or thresholds.
* [series_less](/apl/series-functions/series_less): Returns a Boolean array showing where elements in the first array are less than the second. Use this to detect drops or underperforming values.
* [series_equals_any](/apl/series-functions/series_equals_any): Returns a Boolean indicating if any element in two arrays matches. Use this when you need a single result rather than an array.
* [series_not_equals_any](/apl/series-functions/series_not_equals_any): Returns a Boolean indicating if any element in two arrays differs. Use this when you only need to know if a difference exists.
