...
The `series_not_equals` function compares two numeric arrays element by element and returns a new array of Boolean values. Each element in the output array indicates whether the corresponding elements in the input arrays are not equal. 
...
\[Run in Playground]\([https://play.axiom.co/axiom-play-qf1k/query?initForm=%7B%22apl%22%3A%22%5B'sample-http-logs'%5D](https://play.axiom.co/axiom-play-qf1k/query?initForm=%7B%22apl%22%3A%22%5B'sample-http-logs'%5D) %7C summarize durations1%20%3D%20make\_list(req\_duration\_ms)%20by%20method %7C join%20(%20%20%20%5B'sample-http-logs'%5D %20%20%20%7C%20summarize%20durations2%20%3D%20make\_list(req\_duration\_ms)%20by%20method )%20on%20method %7C extend%20diff\_flags%20%3D%20series\_not\_equals(durations1%2C%20durations2)%22%7D)
...
\[Run in Playground]\([https://play.axiom.co/axiom-play-qf1k/query?initForm=%7B%22apl%22%3A%22%5B'otel-demo-traces'%5D](https://play.axiom.co/axiom-play-qf1k/query?initForm=%7B%22apl%22%3A%22%5B'otel-demo-traces'%5D) %7C where%20service.name%20%3D%3D%20'frontend' %7C summarize%20frontend\_durations%20%3D%20make\_list(duration)%20by%20trace\_id %7C join%20(%20%20%20%5B'otel-demo-traces'%5D %20%20%20%7C%20where%20service.name%20%3D%3D%20'checkoutservice' %20%20%20%7C%20summarize%20checkout\_durations%20%3D%20make\_list(duration)%20by%20trace\_id )%20on%20trace\_id %7C extend%20diff\_flags%20%3D%20series\_not\_equals(frontend\_durations%2C%20checkout\_durations)%22%7D)
...
\[Run in Playground]\([https://play.axiom.co/axiom-play-qf1k/query?initForm=%7B%22apl%22%3A%22%5B'sample-http-logs'%5D](https://play.axiom.co/axiom-play-qf1k/query?initForm=%7B%22apl%22%3A%22%5B'sample-http-logs'%5D) %7C where%20geo.country%20%3D%3D%20'US' %7C summarize%20us\_statuses%20%3D%20make\_list(status)%20by%20uri %7C join%20(%20%20%20%5B'sample-http-logs'%5D %20%20%20%7C%20where%20geo.country%20%3D%3D%20'DE' %20%20%20%7C%20summarize%20de\_statuses%20%3D%20make\_list(status)%20by%20uri )%20on%20uri %7C extend%20diff\_flags%20%3D%20series\_not\_equals(us\_statuses%2C%20de\_statuses)%22%7D)
...
* [series\_equals](/apl/series-functions/series_equals): Returns a Boolean array showing where two arrays are equal element by element. Use this when you want to find matches instead of mismatches.
* [series\_greater](/apl/series-functions/series_greater): Returns a Boolean array showing where elements in the first array are greater than the second. Use this to compare performance metrics or thresholds.
* [series\_less](/apl/series-functions/series_less): Returns a Boolean array showing where elements in the first array are less than the second. Use this to detect drops or underperforming values.
* [series\_equals\_any](/apl/series-functions/series_equals_any): Returns a Boolean indicating if any element in two arrays matches. Use this when you need a single result rather than an array.
* [series\_not\_equals\_any](/apl/series-functions/series_not_equals_any): Returns a Boolean indicating if any element in two arrays differs. Use this when you only need to know if a difference exists.
