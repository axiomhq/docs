...
[Run in Playground]([https://play.axiom.co/axiom-play-qf1k/query?initForm=%7B%22apl%22%3A%22%5B'sample-http-logs'%5D](https://play.axiom.co/axiom-play-qf1k/query?initForm=%7B%22apl%22%3A%22%5B'sample-http-logs'%5D) | summarize durations = make_list(req_duration_ms) by id | extend threshold = dynamic([100,100,100]) | extend exceeds = series_greater_equals(durations, threshold)%22%7D)
...
[Run in Playground]([https://play.axiom.co/axiom-play-qf1k/query?initForm=%7B%22apl%22%3A%22%5B'otel-demo-traces'%5D](https://play.axiom.co/axiom-play-qf1k/query?initForm=%7B%22apl%22%3A%22%5B'otel-demo-traces'%5D) | where ['service.name'] == 'checkoutservice' | summarize durations = make_list(duration) by trace_id | extend baseline = dynamic([100ms,200ms,300ms]) | extend slower = series_greater_equals(durations, baseline)%22%7D)
...
[Run in Playground]([https://play.axiom.co/axiom-play-qf1k/query?initForm=%7B%22apl%22%3A%22%5B'sample-http-logs'%5D](https://play.axiom.co/axiom-play-qf1k/query?initForm=%7B%22apl%22%3A%22%5B'sample-http-logs'%5D) | where status == '500' | summarize fails = make_list(req_duration_ms) by geo.country | extend threshold = dynamic([200,200,200]) | extend suspicious = series_greater_equals(fails, threshold)%22%7D)
...
* [series_equals](/apl/array-functions/series_equals): Compares two arrays element by element and returns `true` if elements are equal.
* [series_less](/apl/array-functions/series_less): Compares two arrays and returns `true` where the first array element is less than the second.
* [series_less_equals](/apl/array-functions/series_less_equals): Compares two arrays and returns `true` where the first array element is less than or equal to the second.
* [series_greater](/apl/array-functions/series_greater): Compares two arrays and returns `true` where the first array element is greater than the second.
* [series_not_equals](/apl/array-functions/series_not_equals): Compares two arrays and returns `true` where elements are not equal.
