...
Use the `hash` scalar function to transform any deterministic, hash-able value into a 64-bit signed integer. The result is stable for identical input values across your entire cluster, so you can:
...
The 64-bit signed integer hash of `source` (and `salt` if supplied).
