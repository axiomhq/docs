...
[Run in Playground]([https://play.axiom.co/axiom-play-qf1k/query?initForm=%7B%22apl%22%3A%22%5B'sample-http-logs'%5D](https://play.axiom.co/axiom-play-qf1k/query?initForm=%7B%22apl%22%3A%22%5B'sample-http-logs'%5D) %7C extend is_ascii_uri%20%3D%20isascii(uri) %7C summarize%20count()%20by%20is_ascii_uri%22%7D)
...
[Run in Playground]([https://play.axiom.co/axiom-play-qf1k/query?initForm=%7B%22apl%22%3A%22%5B'otel-demo-traces'%5D](https://play.axiom.co/axiom-play-qf1k/query?initForm=%7B%22apl%22%3A%22%5B'otel-demo-traces'%5D) %7C extend is_ascii_span_id%20%3D%20isascii(span_id) %7C summarize%20count()%20by%20is_ascii_span_id%22%7D)

**Output**

| is_ascii_span_id | count |
| ------------------- | ----- |
| true                | 28700 |
| false               | 2     |
...
[Run in Playground]([https://play.axiom.co/axiom-play-qf1k/query?initForm=%7B%22apl%22%3A%22%5B'sample-http-logs'%5D](https://play.axiom.co/axiom-play-qf1k/query?initForm=%7B%22apl%22%3A%22%5B'sample-http-logs'%5D) %7C extend is_ascii_id%20%3D%20isascii(id) %7C summarize%20count()%20by%20is_ascii_id%22%7D)
...
- [isnotempty](/apl/scalar-functions/isnotempty): Returns `true` if the string is not empty. Use when checking presence of a value instead of character encoding.
- [isempty](/apl/scalar-functions/isempty): Returns `true` if the string is empty. Use when validating missing or empty input.
- [strlen](/apl/scalar-functions/strlen): Returns the number of characters in a string. Combine with `isascii` to analyze encoding length differences.
- [extract](/apl/scalar-functions/extract): Extracts substrings using regex. Use when parsing parts of ASCII/non-ASCII content.
- [replace_string](/apl/scalar-functions/replace_string): Replaces substrings. Useful for cleaning or normalizing non-ASCII input before validation.
