...
- [string_to_guid](/apl/string-functions/string_to_guid): Converts a string to a GUID. Use it when you need stable hash-like representations, not character analysis.
- [array_length](/apl/array-functions/array_length): Returns the number of elements in an array. Use it to check how many code points a string contains.
- [array_concat](/apl/array-functions/array_concat): Combines multiple arrays. Useful when merging code point arrays from different strings.
- [strlen](/apl/string-functions/strlen): Returns the length of a string in characters, not code points. Use it when you don’t need to inspect Unicode values.
- [parse_url](/apl/string-functions/parse_url): Parses a URL string into components. Use it with `unicode_codepoints_from_string` when decoding or inspecting URL paths.
