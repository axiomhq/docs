...
[Run in Playground](https://play.axiom.co/axiom-play-qf1k/query?initForm=%7B%22apl%22%3A%22let%20attack_pattern%20%3D%20'..\\..\\windows\\system32'%3B%20['sample-http-logs']%20%7C%20where%20uri%20matches%20regex%20regex_quote%28attack_pattern%29%20%7C%20project%20_time%2C%20id%2C%20uri%2C%20status%2C%20['geo.country']%22%7D)
...
This query detects malicious-looking strings literally, without treating `\` as an escape character or `.` as a wildcard.
...
* [matches regex](/apl/string-functions/matches-regex): Matches input against a regex pattern. Use with `regex_quote` when the pattern comes from user input.
* [extract](/apl/string-functions/extract): Extracts values using a regex pattern. Use `regex_quote` to prepare user-supplied patterns safely.
* [replace\_regex](/apl/string-functions/replace-regex): Replaces substrings matching a regex pattern. Combine with `regex_quote` for literal patterns.
* [parse\_url](/apl/string-functions/parse-url): Parses components of a URL. Use when working with structured URLs rather than pattern matching.
* [split](/apl/string-functions/split): Splits strings by a delimiter. Use when you don’t need full regex support.
