...
Identify authentication failures and count them by user and country.

**Query**

```kusto
['sample-http-logs']
| extend is_auth_failure = iff(status == '401' or status == '403', true, false)
| where is_auth_failure
| summarize failed_requests = count() by id, ['geo.country']
| sort by failed_requests desc
```

[Run in Playground](https://play.axiom.co/axiom-play-qf1k/query?initForm=%7B%22apl%22%3A%22%5B%27sample-http-logs%27%5D%20%7C%20extend%20is_auth_failure%20%3D%20iff%28status%20%3D%3D%20%27401%27%20or%20status%20%3D%3D%20%27403%27%2C%20true%2C%20false%29%20%7C%20where%20is_auth_failure%20%7C%20summarize%20failed_requests%20%3D%20count%28%29%20by%20id%2C%20%5B%27geo.country%27%5D%20%7C%20sort%20by%20failed_requests%20desc%22%7D)

**Output**

| id | geo.country | failed_requests |
| -- | ----------- | --------------- |
| u_8821 | DE | 47 |
| u_4403 | US | 32 |
| u_1192 | RU | 28 |

The query flags auth failures with a Boolean column, filters to only those rows, and then groups by user and country to reveal which accounts generate the most unauthorized requests.
