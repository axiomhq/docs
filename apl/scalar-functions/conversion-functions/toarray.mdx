...
* [parse\_json](/apl/utility-functions/parse_json): Converts a string containing JSON into a dynamic-typed value. Use it before `toarray` when your data is in string form.
...
* [array\_length](/apl/array-functions/array_length): Returns the number of elements in an array. Use it with `toarray` to count values.
* [array\_index\_of](/apl/array-functions/array_index_of): Finds the index of a value in an array. Use it when searching for specific elements.
* [bag\_keys](/apl/bag-and-property-bag-functions/bag_keys): Returns an array of property names from a bag. Use it as an alternative to `toarray` when you want the keys instead of values.
