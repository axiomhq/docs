---
title: 'Special field attributes'
description: 'This page explains how to implement special fields within APL queries to enhance the functionality and interactivity of datasets. Use these fields in APL queries to add unique behaviors to the Axiom user interface.'
tags: ['axiom documentation', 'documentation', 'axiom', 'APL', 'tutorial', 'apl overview', 'row attributes hints', 'column attributes hints', 'fieldname unit', 'row url', 'row status', 'row description', 'col tooltip', 'col heatmap', 'data visualization', 'user interface']
---

## Add a link to a table

- Name: `_row_url`
- Type: string
- Description: Defines the URL to which the entire table links.
- APL query example: `extend _row_url = 'https://axiom.co/'`
- Expected behavior: Makes rows clickable. When clicked, goes to the specified URL.

If you specify a static string as the URL, all rows link to that page. To specify a different URL for each row, use a dynamic expression like `extend _row_url = strcat('https://axiom.co/', uri)` where `uri` is a field in your data.

## Add a link to values in a field

- Name: `_FIELDNAME_url`
- Type: string
- Description: Defines a URL to which values in a field link.
- APL query example: `extend _website_url = 'https://axiom.co/'`
- Expected behavior: Makes values in the `website` field clickable. When clicked, goes to the specified URL.

Replace `FIELDNAME` with the actual name of the field.

## Add a tooltip to values in a field

- Name: `_FIELDNAME_tooltip`
- Type: string
- Description: Defines text to be displayed when hovering over values in a field.
- Example Usage: `extend _errors_tooltip = 'Number of errors'`
- Expected behavior: Displays a tooltip with the specified text when the user hovers over values in a field.

Replace `FIELDNAME` with the actual name of the field.

## Add a description to values in a field

- Name: `_FIELDNAME_description`
- Type: string
- Description: Defines additional information to be displayed under the values in a field.
- Example Usage: `extend _diskusage_description = 'Current disk usage'`
- Expected behavior: Displays additional text under the values in a field for more context.

Replace `FIELDNAME` with the actual name of the field.

## Example

The example APL query below adds a tooltip and a description to the values of the `status` field. Clicking one of the values in this field leads to a page about status codes.

```apl
['http-logs']
| extend _status_tooltip = "The status of the HTTP request is the response code from the server. It shows if an HTTP request has been successfully completed."
| extend _status_description = "This is the status of the HTTP request."
| extend _status_url = "https://developer.mozilla.org/en-US/docs/Web/HTTP/Status"
```