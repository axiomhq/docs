...
1. In **Field name**, enter the full name of the field, including parent fields, if any. For example, `map_field.property1.property2`. For more information on syntax, see [Access properties of nested maps](#access-properties-of-nested-maps)
1. Click **Create map field**.

## View unused and removed map fields

To view unused and removed map fields:
1. Go to the Datasets tab.
1. Select a dataset where you want to view map fields.
1. Unused and removed map fields are labelled in the following way:

- **UNUSED** means that the field is configured as a map field but you haven’t yet ingested data into it.
- **REMOVED** means that the field was configured as a map field but at some point it was changed to a normal field. Axiom adds new events to the field normally. Events you ingested before the change retain the map structure.