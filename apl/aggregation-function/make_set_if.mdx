...
- **`summarize`**: The general-purpose aggregation function for summarizing data, which can be combined with conditional filters.
- **`count_if`**: Counts rows that satisfy a specific condition, useful for when you need to count rather than aggregate distinct values.
- **`arg_max_if`**: Finds the row with the maximum value based on a condition, useful when you want to find the highest value under specific criteria.