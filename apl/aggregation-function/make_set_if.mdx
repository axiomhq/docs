...
- [`make_list_if`](/apl/aggregation-function/make_list_if): Similar to `make_set_if`, but returns a list that can include duplicates instead of a distinct set.
- [`make_set`](/apl/aggregation-function/make_set): Aggregates distinct values without a conditional filter.
