...
<Note>
The `dcount` aggregation in APL is a statistical aggregation that gives estimated results. The estimation comes with the benefit of speed at the expense of accuracy. This means that `dcount` is fast and light on resources even on a large or high-cardinality dataset, but it doesn’t provide precise results.
</Note>
