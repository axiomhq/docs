...
- **`arg_max`**: Returns the row with the maximum value for a given field, along with its associated columns. Use `arg_max` when you need the maximum value instead of a list of values.
- **`make_bag_if`**: Creates a dictionary of key-value pairs based on a condition. Use `make_bag_if` when you need key-value aggregation with conditional logic.
