...
```kusto
requests
| summarize avgif(response_time, response_time > 500) by bin(timestamp, 1m)
```
...
This query calculates the average response time for requests that took more than 500 milliseconds, grouped by one-minute intervals.
