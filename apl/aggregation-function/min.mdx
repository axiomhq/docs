...
The `min` aggregation function in APL gets the minimum value from a set of input values. You can use this function to identify the smallest numeric or comparable value in a column of data. This is useful when you want to find the quickest response time, the lowest transaction amount, or the earliest date in log data. It's ideal for analyzing performance metrics, filtering out abnormal low points in your data, or discovering outliers.
...
The function gets the smallest value found in the specified column or expression.
...
- [**max**](/apl/aggregation-function/max): Gets the maximum value from a set of values. Use `max` when you need to find the highest value instead of the lowest.
...
- [**percentile**](/apl/aggregation-function/percentile): Gets the value at a specified percentile. Use `percentile` if you need a value that falls at a certain point in the distribution of your data, rather than the minimum.