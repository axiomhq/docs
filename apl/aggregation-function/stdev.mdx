...
In Splunk SPL, the `stdev` aggregation function works similarly but has a different syntax. While SPL uses the `stdev` command within the `stats` function, APL users will find that the aggregation works similarly in APL with just minor differences in syntax.
...
This query calculates the standard deviation of the `req_duration_ms` field in the `sample-http-logs` dataset, helping you understand how much variability exists in request durations.
