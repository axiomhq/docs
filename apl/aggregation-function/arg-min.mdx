...
<Note>
The wildcard `*` is useful to return all fields from the row with the minimum value, but it increases query complexity and decreases performance. For more information, see [Wildcards](/apl/reference/wildcards).
</Note>
