...
In log analysis, you can use the `countif` function to count the number of log entries based on their country of origin.

**Query**

```kusto
['sample-http-logs']
| summarize countif(['geo.country'] == "France") by bin(_time, 1m)
```
...
This query counts the number of log entries that originate in France in one-minute intervals.
