...
| `field1, field2` | The additional fields to retrieve from the record with the maximum numeric value. You can use `*` as a wildcard to return all fields from the row. |

<Note>
Using `arg_max(expression, *)` returns all fields from the row with the maximum value, which can be useful when you need full context but may increase query complexity with wide datasets.
</Note>
