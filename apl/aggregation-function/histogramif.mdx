...
['sample-http-logs']
| summarize histogramif(req_duration_ms, status == '200', 10)
```
</CodeGroup>
...
```kusto
histogramif(numeric_field, condition, number_of_bins)
```
...
```kusto
['sample-http-logs']
| summarize histogramif(req_duration_ms, status == '200', 100) by bin_auto(_time)
```

[Run in Playground](https://play.axiom.co/axiom-play-qf1k/query?initForm=%7B%22apl%22%3A%22%5B'sample-http-logs'%5D%20%7C%20summarize%20histogramif(req_duration_ms%2C%20status%20%3D%3D%20'200'%2C%20100)%20by%20bin_auto(_time)%22%7D)
...
```kusto
['otel-demo-traces']
| summarize histogramif(duration, ['service.name'] == 'frontend', 50) by bin_auto(_time)
```

[Run in Playground](https://play.axiom.co/axiom-play-qf1k/query?initForm=%7B%22apl%22%3A%22%5B'otel-demo-traces'%5D%20%7C%20summarize%20histogramif(duration%2C%20%5B'service.name'%5D%20%3D%3D%20'frontend'%2C%2050)%20by%20bin_auto(_time)%22%7D)
...
```kusto
['sample-http-logs']
| summarize histogramif(req_duration_ms, ['geo.country'] == 'US', 50) by bin_auto(_time)
```

[Run in Playground](https://play.axiom.co/axiom-play-qf1k/query?initForm=%7B%22apl%22%3A%22%5B'sample-http-logs'%5D%20%7C%20summarize%20histogramif(req_duration_ms%2C%20%5B'geo.country'%5D%20%3D%3D%20'US'%2C%2050)%20by%20bin_auto(_time)%22%7D)
