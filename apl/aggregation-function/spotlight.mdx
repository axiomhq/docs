...
| `Field1 ... FieldN` | field references   | One or more fields to analyze. Include string or categorical fields (for proportions) and numeric or timespan fields (for distributional differences). You can use `*` as a wildcard to analyze all fields. |

<Note>
Using `spotlight(predicate, *)` to analyze all fields can be expensive with wide datasets. Consider specifying only relevant fields when possible.
</Note>
