...
<Note>
The `topk` aggregation in APL is a statistical aggregation with maximum error of 1%. The estimation comes with the benefit of speed at the expense of accuracy. This means that `topk` is fast and light on resources even on a large or high-cardinality dataset, but it doesn’t provide the most accurate results.
