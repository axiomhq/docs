...
In Splunk SPL, the equivalent of the `topk` function is the `top` command. The `topk` function in APL behaves similarly by returning the top *k* values of a specified field, but its syntax is unique to APL.

The main difference between SPL’s `top` and APL’s `topk` is that `topk` is estimated. This means that APL’s `topk` is faster, less resource intenstive, but less accurate than SPL’s `top`.
