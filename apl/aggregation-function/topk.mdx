...
- **`value`**: The field or expression to rank the results by.
- **`ties`** *(optional)*: Specifies whether to include ties for the last place. Default is `false`.

### Returns

A subset of the original dataset with the top *k* values based on the specified field. If `ties` is `true`, records with the same ranking at the boundary of the result set are included.
...
[Run in Playground](https://play.axiom.co/axiom-play-qf1k/explorer?initForm=%7B%22apl%22%3A%22%5B%27sample-http-logs%27%5D%20%7C%20summarize%20count%28%29%20by%20status%20%7C%20topk%285%2C%20count_%29%22%7D)
...
[Run in Playground](https://play.axiom.co/axiom-play-qf1k/explorer?initForm=%7B%22apl%22%3A%22%5B%27sample-http-logs%27%5D%20%7C%20summarize%20count%28%29%20by%20%5B%27geo.city%27%5D%20%7C%20topk%285%2C%20count_%29%22%7D)
