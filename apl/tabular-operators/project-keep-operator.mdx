...
The `project-keep` operator in APL is a powerful tool for column selection. It allows you to explicitly keep specific columns from a dataset, discarding any others not listed in the operator's parameters. This is useful when you only need to work with a subset of fields in your query results and want to reduce clutter or improve performance by eliminating unnecessary columns. 
...
In Splunk SPL, the `table` command performs a similar task to APL’s `project-keep`. It selects only the columns you specify and excludes any others.
...
In ANSI SQL, the `SELECT` statement combined with column names performs a task similar to `project-keep` in APL. Both allow you to specify which columns to retrieve from the dataset.
...
```kusto
| project-keep ColumnName1, ColumnName2, ...
```

### Parameters

- `ColumnName`: The column you want to keep in the result set.

### Returns

`project-keep` returns a dataset with only the specified columns. All other columns are removed from the output.
...
For log analysis, you might want to keep only the columns that are relevant to investigating HTTP requests.
...
- [**`project`**](/apl/tabular-operators/project-operator): Use `project` to explicitly specify the columns you want in your result, while also allowing transformations or calculations on those columns.
- [**`extend`**](/apl/tabular-operators/extend-operator): Use `extend` to add new columns or modify existing ones without dropping any columns.
