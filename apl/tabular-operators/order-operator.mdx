...
In Splunk SPL, the equivalent operator to `order` is `sort`. SPL uses a similar syntax to APL but with some differences. In SPL, `sort` allows both ascending (`asc`) and descending (`desc`) sorting, while in APL, you achieve sorting using the `asc()` and `desc()` functions for columns.
...
In ANSI SQL, the equivalent of `order` is `ORDER BY`. SQL uses `ASC` for ascending and `DESC` for descending order. In APL, sorting works similarly, with the `asc()` and `desc()` functions added around column names to specify the order.
...
- `ColumnName`: The name of the column by which to sort.
- `asc`: Sorts the column in ascending order.
- `desc`: Sorts the column in descending order.

### Returns

The `order` operator returns the input dataset, sorted according to the specified columns and order (ascending or descending). If multiple columns are specified, sorting is done based on the first column, then by the second if values in the first column are equal, and so on.
