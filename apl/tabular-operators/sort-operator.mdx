...
When you use `project` and `sort` in the same query, ensure that you project the fields that you want to sort on. Similarly, when you use `project-away` and `sort` in the same query, ensure that you don't remove the fields that you want to sort on.

The above is also true for time fields. For example, to project the field `status` and sort on the field `_time`, project both fields as shown in the query below:
...
- [**extend**](/apl/tabular-operators/extend-operator): Use `extend` to create calculated fields that you can then use with `sort` to refine your results.
