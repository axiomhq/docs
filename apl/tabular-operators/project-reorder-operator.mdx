...
The `project-reorder` operator in APL allows you to rearrange the fields of a data set without modifying the underlying data. This operator is useful when you need to control the display order of fields in query results, making your data easier to read and analyze. It's especially helpful when working with large data sets where field ordering impacts the clarity of the output.
...
In ANSI SQL, the order of fields in a `SELECT` statement determines their arrangement in the output. In APL, `project-reorder` provides more explicit control over field order without requiring a full `SELECT` clause.
...
- `[asc | desc | granny-asc | granny-desc]`: Optional: Specifies the sort order for the reordered fields. `asc` or `desc` order fields by field name in ascending or descending order. `granny-asc` or `granny-desc` order by ascending or descending while secondarily sorting by the next numeric value. For example, `b50` comes before `b9` when you use `granny-asc`.

### Returns

A table with the specified fields reordered as requested, followed by any unspecified fields in their original order. `project-reorder` doesn't rename or remove fields from the data set. All fields that existed in the data set appear in the results table.
...
A wildcard is a special character or set of characters that can substitute for any other character in a search pattern. Use wildcards to create more flexible queries and perform more powerful searches.

The syntax for a wildcard can be either `data*` or `['data.fo']*`.
...
```kusto
['otel-demo-traces']
| project-reorder trace_id, *, span_id // Orders the trace_id first, followed by everything else, then span_id fields 
```
...
- [**sort**](/apl/tabular-operators/sort-operator): Sorts rows based on field values. The results can then be reordered with `project-reorder`.