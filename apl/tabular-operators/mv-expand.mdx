...
```kusto
T | mv-expand ColumnName [to typeof(DataType)] [limit=N]
```
...
| --------------------- | ------------------------------------------------------------------------------- |
| `ColumnName`          | The name of the column that contains a dynamic array or property bag to expand. |
| `to typeof(DataType)` | Optional. Converts each expanded element to the specified type.                 |
| `limit=N`             | Optional. Limits the number of expanded rows per record.                        |

### Returns

The operator returns a table where each element of the expanded column is placed in its own row. Other columns are duplicated for each expanded row.
...
- [expand](/apl/tabular-operators/expand-operator): Expands JSON objects into rows. Use it when you want to expand structured data, not arrays.
...
- [parse_json](/apl/scalar-functions/parse-json-function): Converts JSON strings to dynamic objects. Use it before `mv-expand` when working with string-encoded arrays.
