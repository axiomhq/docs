...
<Info>
The `externaldata` operator retrieves data from **static files** in cloud storage or HTTP endpoints. It doesn't support live connections to relational databases such as PostgreSQL, MySQL, or other SQL databases. If you want to correlate Axiom data with data from a relational database, export the relevant data from your database to a file in cloud storage (such as an S3 bucket or Azure Blob Storage) and reference it with `externaldata`.
</Info>
