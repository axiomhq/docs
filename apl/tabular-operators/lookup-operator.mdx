...
The `lookup` operator extends a primary dataset with a lookup dataset based on a specified key column. It retrieves matching rows from the lookup dataset and appends relevant fields to the primary dataset. You can use `lookup` for enriching event data, adding contextual information, or correlating logs with reference tables.
...
In Splunk SPL, the `lookup` command performs a similar function by enriching event data with fields from an external lookup table.

<CodeGroup>
```sql Splunk example
index=web_logs | lookup geoip_lookup ip AS client_ip OUTPUT city, country
```

```kusto APL equivalent
['sample-http-logs']
| lookup kind=inner ['geoip_lookup'] on id
```
</CodeGroup>
...
In ANSI SQL, `lookup` is similar to an `INNER JOIN`, where records from both tables are matched based on a common key.

<CodeGroup>
```sql SQL example
SELECT logs.*, geo.city, geo.country
FROM logs
INNER JOIN geoip_lookup geo ON logs.id = geo.id;
```
...
- `PrimaryDataset`: The primary dataset that you want to extend. If you expect one of the datasets to contain consistently more data than the other, specify the larger dataset as the left side of the join.
- `LookupTable`: The secondary dataset containing additional data, also known as the dimension table or lookup table.
...
- `Conditions`: The conditions for matching rows from `PrimaryDataset` to rows from `LookupTable`. The conditions are equality expressions that determine how Axiom matches rows from the `PrimaryDataset` (left side of the equality expression) with rows from the `LookupTable` (right side of the equality expression). The two sides of the equality expression must have the same data type.
    - To use `lookup` on a key column that has the same name in the two datasets, simply use the field name. For example, `on id`.
    - To use `lookup` on a key column that has different names in the two datasets, define the two field names in an equality expression such as `on id == trace_id`.
