...
In this use case, you extract the extension from the `content_type` field for valid log entries.
...
| _time               | content_type | extension |
|---------------------|--------------|-----------|
| 2023-10-01 12:00:00 | text/html     | html       |
| 2023-10-01 12:01:00 | NULL         | NULL      |
| 2023-10-01 12:02:00 | image/gif       | gif    |
| 2023-10-01 12:03:00 | application/octet-stream       | octet    |
| 2023-10-01 12:03:00 | image/123      | NULL    |


In this query, the `extract` function retrieves the extension from the `content_type` field for valid log entries. If the `content_type` field is null or starts with a non-alphabetical character, no extension is extracted, and the result remains null.
