...
| **DatasetPattern** | string |          | A wildcard pattern to match datasets. For more information, see [Wildcards](/apl/reference/wildcards).|
...
This searches for the term `error` across all datasets that start with `github`, such as `github-push-event` and `github-pull-request-event`.

For more information, see [Wildcards](/apl/reference/wildcards).
