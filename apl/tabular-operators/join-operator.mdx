...
- `Conditions`: The conditions for matching rows. The conditions determine how Axiom matches rows from the `LeftDataset` with rows from the `RightDataset`.
    ...
    - You can use expressions in the join conditions. For example, to compare two fields of different data types, use `on $left.id_string == tostring(trace_id_int)`.
    - You can define multiple join conditions. To separate conditions, use commas (`,`). Don’t use `and`. For example, `on $left.id == $right.trace_id, $left.span == $right.span_id`.
