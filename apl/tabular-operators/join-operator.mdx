...
<Warning>
The preview of the `join` operator currently only supports `inner` join. If you attempt to use other join kinds like `leftouter`, `rightouter`, or `fullouter`, your query will fail with an error. Support for other kinds of join is coming soon.
</Warning>

<Tip>
While waiting for additional join types, you can use `kind=leftouter` workarounds with the `lookup` operator for some use cases. See the [lookup operator](/apl/tabular-operators/lookup-operator) documentation for more information.
</Tip>
