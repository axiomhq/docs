...
The `project` operator in Axiom Processing Language (APL) is used to select specific columns from a dataset, potentially renaming them or applying calculations on the fly. With `project`, you can control which columns are returned by the query, allowing you to focus on only the data you need.

This operator is useful when you want to refine your query results by reducing the number of columns, renaming them, or deriving new columns based on existing data. It’s a powerful tool for filtering out unnecessary fields and performing light transformations on your dataset.
...
In Splunk SPL, the equivalent of the `project` operator is typically the `table` or `fields` command. While SPL’s `table` focuses on selecting columns, `fields` controls both selection and exclusion, similar to `project` in APL.
...
In ANSI SQL, the `SELECT` statement serves a similar role to the `project` operator in APL. SQL users will recognize that `project` behaves like selecting columns from a table, with the ability to rename or transform columns inline.
...
- `FieldName`: The names of the columns in the order you want them to appear in the result set. If there is no Expression, then FieldName is compulsory and a column of that name must appear in the input.
- `Expression`: Optional scalar expression referencing the input columns.

### Returns

The `project` operator returns a dataset containing only the specified columns.
...
The query returns only the timestamp, HTTP status code, and request URI, reducing unnecessary columns from the dataset.
...
- [**`extend`**](/apl/tabular-operators/extend-operator): Use `extend` to add new columns or calculate values without removing any existing columns.
