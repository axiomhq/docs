...
This example combines the datasets and then filters the data to include only rows where the `method` is `GET`.
...
This query combines GitHub pull request event logs and GitHub push events, filters by actions made by `github-actions[bot]`, and displays key event details such as `time`, `repository`, `commits`, `head`, and `id`.
...
This example removes the `content_type` and `commits` fields in the datasets `sample-http-logs` and `github-push-event` before combining the datasets.
...
This example performs a union and then filters the resulting set to include only rows where the `method` is `GET`.
