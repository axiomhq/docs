...
<Warning>
Using `union *` to query all datasets is extremely expensive and should be avoided in production. Prefer specific dataset names or prefix patterns when possible.
</Warning>
...
- Avoid using `union *` in production as it queries all datasets and is extremely expensive. Use specific dataset names or prefix patterns (e.g., `github*`) instead.