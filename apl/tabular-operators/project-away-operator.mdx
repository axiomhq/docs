...
The `project-away` operator in APL excludes specific fields from the output of a query. This operator is useful when you want to return a subset of fields from a dataset without needing to manually specify every field you want to keep. Instead, you specify the fields you want to remove, and the operator returns all remaining fields.

Use `project-away` in scenarios where your dataset contains irrelevant or sensitive fields that you don't want in the results. It simplifies queries, especially when dealing with wide datasets, by allowing you to filter out fields without explicitly listing every field to include.
...
In Splunk SPL, you use the `fields` command to remove fields from your results. In APL, the `project-away` operator provides similar functionality, removing specified fields while returning the remaining ones.
...
In SQL, you typically use the `SELECT` statement to explicitly include fields. In contrast, APL's `project-away` operator lets you exclude fields, offering a more concise approach when you want to keep many fields but remove a few.
...
A wildcard is a special character or a set of characters that can substitute for any other character in a search pattern. Use wildcards to create more flexible queries and perform more powerful searches.
...
- [**extend**](/apl/tabular-operators/extend-operator): The `extend` operator adds new fields, whereas `project-away` removes fields.
- [**summarize**](/apl/tabular-operators/summarize-operator): While `project-away` removes fields, `summarize` aggregates data across multiple fields.