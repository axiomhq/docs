...
- [**`extend`**](/apl/tabular-operators/extend-operator): Use the `extend` operator when you want to add calculated columns without parsing text.
- [**`project`**](/apl/tabular-operators/project-operator): Use `project` to select and rename columns after parsing text.