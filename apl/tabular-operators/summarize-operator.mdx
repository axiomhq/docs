...
The `summarize` operator in APL enables you to perform data aggregation and create summary tables from large datasets. You can use it to group data by specified columns and apply aggregation functions such as `count()`, `sum()`, `avg()`, `min()`, `max()`, and many others. This is particularly useful when analyzing logs, tracing OpenTelemetry data, or reviewing security events. The `summarize` operator is helpful when you want to reduce the granularity of a dataset to extract insights or trends.
...
The `summarize` operator in APL is conceptually similar to SQL’s `GROUP BY` clause with aggregation functions. In APL, you explicitly specify the aggregation function (like `count()`, `sum()`) and the columns to group by.
...
```kusto
| summarize [[Column1 =] AggregationFunction [, ...]] [by [Column2 =] GroupExpression [, ...]]
```

### Parameters

- `Column1`: A column name.
...
- [**count**](/apl/tabular-operators/count-operator): Use when you only need to count rows without grouping by specific columns.
- [**extend**](/apl/tabular-operators/extend-operator): Use to add new calculated columns to a dataset.
- [**project**](/apl/tabular-operators/project-operator): Use to select specific columns or create new calculated columns, often in combination with `summarize`.