...
<Note>
**PromQL support:** Axiom does not support PromQL (Prometheus Query Language) queries. If you're migrating from Prometheus or need to translate PromQL queries to APL, use APL's aggregation functions and operators instead. For metrics queries, Axiom supports OpenTelemetry metrics with functions like `prom::rate` for Prometheus-style rate calculations. For help translating specific PromQL queries to APL, see the [APL tutorial](/apl/tutorial) or contact support.
</Note>
