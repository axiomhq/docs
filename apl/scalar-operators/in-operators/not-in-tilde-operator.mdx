...
<Note>
Case-insensitive operators are supported only for ASCII text. For non-ASCII comparison, use the [tolower()](/apl/scalar-functions/string-functions#tolower) function.
</Note>
...
| ---------- | ----------------- | -------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| Expression | scalar            | Yes      | The expression whose value to check for non-membership (case-insensitive).                                                                                         |
| Value      | scalar or tabular | Yes      | A scalar value or tabular expression that specifies the set of values to exclude. If a tabular expression has multiple columns, the first column is used. The search considers up to 1,000,000 distinct values. |
...
Exclude requests from specific countries regardless of case variations in the data.
...
| 2024-10-17 09:40:00 | user3 | /admin    | POST   | Brazil      | São Paulo |

This query filters out requests from specified countries regardless of case, allowing security analysts to focus on traffic from other regions.
...
Nested arrays are flattened into a single list of values. For example, `x !in~ (dynamic(['a', ['b', 'c']]))` becomes `x !in~ ('a', 'b', 'c')`.
