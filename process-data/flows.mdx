...
1. Click the **Flows** tab. Axiom displays the list of flow conigurations you have created. Select the flow configuration that you want to use for creating a continuous flow.
...
<Note>
The delivery rate of a continuous flow currently depends on the ingest rate at the source dataset. For a continuous flow defined on a source that's ingesting events at a rate of 1TB/day, events will be processed within a few seconds of ingest. For a source ingesting events at a rate of 10GB/day, it can take several minutes for events to arrive. There is currently no maximum wait time for events to be processed by a continuous flow. As Flow progresses through Preview, maximum wait times will be established and refined.
</Note>