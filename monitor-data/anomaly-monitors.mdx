...
1. The preview shows on the resulting chart the boundary where the monitor will trigger as a dashed line. The portion of the chart where there is not enough data to compute a boundary is grayed out. If the monitor preview shows that it would have alerted during times when it should not have, try increasing the tolerance. Similarly, decrease the tolerance if the monitor preview shows that it would not have alerted during times when it should have.
1. Click **Create**.
