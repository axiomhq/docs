...
For example, if logs in your dataset `['sample_dataset']` have a `status.code` attribute that takes the value `ERROR` when a log is about an error, the following example query tracks the error rate every minute:
...
Other options:
- To trigger the monitor when the error rate is greater than or equal to 0.01, set the threshold value to 0.01 and the comparison operator to `above or equal`.
...
## Notify when number of error messages exceeds threshold

To receive a notification when the number of error messages of a given type exceeds a threshold, create a threshold monitor with an APL query that counts the different error messages.

For example, if logs in your dataset `['sample_dataset']` have an `error.message` attribute, the following example query counts errors by type every 5 minutes:
...
Other options:
- To trigger the monitor when the count is greater than or equal to 10 for any individual message type, set the threshold to 10 and the comparison operator to **above or equal**.
- To run the monitor every 5 minutes, set the frequency to 5.
- To run the monitor query with a range of 10 minutes, set the range to 10.
...
For example, if you have a dataset `['my_traces']` of trace data with the following:
...
Other options:
- To trigger the monitor when response times are unusually high for a route, set the comparison operator to **above**.
- To run the monitor every 5 minutes, set the frequency to 5.
