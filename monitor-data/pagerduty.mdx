...
- Obtain deep understanding of the issue root cause by visualizing the data using Axiom

Axiom creates PagerDuty events that arise from critical issues, disruptions, vulnerabilities, or workload downtime on a service created in PagerDuty. The alert on the Axiom side is linked to the PagerDuty Event, allowing Axiom to automatically close the Event incident if the Alert is resolved. This ensures no duplicate Events on the PagerDuty side are created for the corresponding ones on the Axiom side.
...
1. Click **PagerDuty**
1. Enter the integration key you previously generated
1. Click **Create**
