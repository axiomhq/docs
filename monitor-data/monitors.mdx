...
To receieve a notification whenever your response times spike without having to rely on a single threshold, create an anomaly monitor with an APL query that tracks your median response time. 

For example, you have a dataset `['my_traces']` of trace data where route information is given in the `route` attribute, duration is given in the `duration` attribute, and top-level spans are identified by those where the attribute `parent_span_id` is empty. The following query gives median response times by route in one minute intervals:
