---
title: 'Configure monitors'
description: 'This page explains how to configure monitors.'
sidebarTitle: Configure
tags:
    ['axiom documentation', 'documentation', 'axiom', 'dashboards', 'monitors', 'notifiers', 'slack', 'email', 'pagerduty', 'webhook', 'opsgenie', 'discord', 'discord webhook']
---

## View monitor status

To view the status of a monitor:
1. Click the Monitors tab.
1. Click the monitor in the list whose status you want to view.

The monitor status page provides an overview of the current status and the history of the monitor.

On the left, you see the recent activity and the history of the monitor runs:
- The `_time` field displays the time of the monitor run.
- The `Status` field displays the status of the monitor.
- The `Range from` and `Range to` fields display the time range used in the monitor run.

You can change the time range of this overview in the top right corner.

On the right, you see details about the monitor’s configuration.
- Current status
- Monitor type
- Query the monitor periodically runs
- Configuration details
- Notifiers attached to the monitor
- Metadata suich as name and description

## Change monitors

To change an existing monitor:
1. Click the Monitors tab.
1. Click the monitor in the list that you want to disable.
1. In the top right, click **Edit monitor**.
1. Make changes to the monitor.
1. Click **Save**.

## Disable monitors

Disable a monitor to prevent it from running for a specific amount of time.

To disable a monitor:
1. Click the Monitors tab.
1. Click the monitor in the list that you want to disable.
1. In the top right, click **Disable monitor**.
1. Select the time period for which you want to disable the monitor.
1. Click **Disable**.

Axiom automatically enables the monitor after the time period you specified.

## Enable monitors

To enable a monitor:
1. Click the Monitors tab.
1. Click the monitor in the list that you want to enable.
1. In the top right, click **Enable monitor**.
1. Click **Enable**.

## Delete monitors

To delete a monitor:
1. Click the Monitors tab.
1. Click the monitor in the list that you want to delete.
1. In the top right, click <img src="/doc-assets/icons/delete.svg" className="inline-icon" alt="Delete icon" />.
1. Click **Delete**.