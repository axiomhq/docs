...
1. Enter a name for your monitor and add a description.
1. Configure the monitor using the following options:
    - **Threshold**: Enter the numeric value to compare against the query results.
    - **Comparison operator**: Select the rule for comparing results to the threshold (**above**, **above or equal**, **below**, or **below or equal**).
    - **Frequency**: Specify how often the monitor runs (in minutes).
    - **Range**: Set the time range for your query (in minutes). The end time is when the monitor runs.
    - **Alert on no data**: Enable this to trigger the monitor when your query returns no data. Your query returns no data if no events match your filters and an aggregation used in the query is undefined. For example, when you take the average of a field not present in any matching events.
    - **Group by attributes**: Use this when defining your query. By default, your monitor enters the alert state if any of the values returned for the group-by attributes cross the threshold, and remains in the alert state until none of the values returned cross the threshold. To trigger the monitor separately for each group that crosses the threshold, enable **Notify by group**. At most one trigger notification is sent per monitor run. This option only has an effect if the monitor's query groups by a non-time field.
