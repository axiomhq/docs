...
<Tab title="EU Central 1 (AWS)">
```bash
LOG_CHANNEL=axiom
AXIOM_API_TOKEN=API_TOKEN
AXIOM_DATASET=DATASET_NAME
AXIOM_DOMAIN=eu-central-1.aws.edge.axiom.co
LOG_LEVEL=debug
LOG_DEPRECATIONS_CHANNEL=null
```
</Tab>
...
<Info>
  ...
  The `AXIOM_DOMAIN` value depends on your edge deployment region:
...
<Info>
  The `AxiomHandler` class uses the ingest endpoint format `https://{AXIOM_DOMAIN}/v1/ingest/{dataset}` which works for all edge deployments (US and EU).

  Make sure to set the correct `AXIOM_DOMAIN` in your `.env` file based on your region:
...
Axiom offers multiple edge deployment regions. Choose the region closest to your application for optimal performance.

| Edge deployment | Base domain | Ingest endpoint |
|-----------------|-------------|-----------------|
| US East 1 (AWS) | `us-east-1.aws.edge.axiom.co` | `https://us-east-1.aws.edge.axiom.co/v1/ingest/{dataset}` |
| EU Central 1 (AWS) | `eu-central-1.aws.edge.axiom.co` | `https://eu-central-1.aws.edge.axiom.co/v1/ingest/{dataset}` |

<Tabs>
<Tab title="US East 1 (AWS)">
Configure your `.env` file for US East 1:
...
Your logs will be sent to:
```
https://us-east-1.aws.edge.axiom.co/v1/ingest/your-dataset-name
```
</Tab>
<Tab title="EU Central 1 (AWS)">
Configure your `.env` file for EU Central 1:
...
Your logs will be sent to:
```
https://eu-central-1.aws.edge.axiom.co/v1/ingest/your-dataset-name
```
</Tab>
</Tabs>

<Info>
  If you're unsure which edge deployment to use, check your Axiom organization settings or contact support. Using the wrong region may result in 403 Forbidden errors when attempting to ingest logs.
</Info>
...
If you receive a 403 Forbidden error when sending logs, check the following:

1. **Incorrect edge deployment**: Ensure the `AXIOM_DOMAIN` in your `.env` file matches your organization's edge deployment region.
2. **Invalid API token**: Verify that your `AXIOM_API_TOKEN` is correct and has ingest permissions for the specified dataset.
3. **Dataset doesn't exist**: Confirm that the dataset specified in `AXIOM_DATASET` exists in your Axiom organization.

<Info>
  Common causes of 403 errors:
  - Using `api.axiom.co` instead of the edge domain (`us-east-1.aws.edge.axiom.co` or `eu-central-1.aws.edge.axiom.co`)
  - API token doesn't have ingest permissions for the specified dataset
  - Organization is on EU but using US endpoint (or vice versa)
</Info>

### Logs not appearing

If logs are not appearing in your Axiom dataset:

1. Check your Laravel logs at `storage/logs/laravel.log` for any cURL errors.
2. Verify that `LOG_CHANNEL` is set to `axiom` in your `.env` file.
3. Clear Laravel's config cache: `php artisan config:clear`
