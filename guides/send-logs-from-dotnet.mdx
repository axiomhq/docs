...
Logging is an important aspect of app development and monitoring. It allows developers to track app behavior, debug issues, and gain insights into the system’s performance. In this guide, this guide explores how to send logs from a .NET app to Axiom. The process is explained step by step, from setting up the project to configuring the Axiom logger and running the app.
...
- An [Axiom account](https://app.axiom.co/) and [API token](https://axiom.co/docs/reference/tokens#api-tokens)
...
To begin, create a new .NET project. Open your terminal or command prompt and navigate to the directory where you want to create your project. Run the following command to create a new console app:

```bash
dotnet new console -n AxiomLogs
```

This command creates a new C# console app project named `AxiomLogs`.

### Installing packages

Next, install the packages for your project. Use the `Microsoft.AspNet.WebApi.Client` package to make HTTP requests to the Axiom API. Run the following command to install the package:
...
Now that the project is set up, create a class to handle logging to Axiom. Create a new file named `AxiomLogger.cs` in your project directory and add the following code:
...
To build and run the app, navigate to the project directory in your terminal and run the following command:
...
This command builds the project and runs the app. You should see the log messages being sent to Axiom, and the console displays `Log sent successfully.` for each log entry.
...
Add Serilog and the necessary extensions to your project. You might need the `Serilog`, `Serilog.Sinks.Http`, and `Serilog.Formatting.Json` packages.
...
To build and run the app, navigate to the project directory in your terminal and run the following commands:
...
- Log contextual data: Include relevant information such as user IDs, request details, and system state in your log messages to provide context when investigating issues.
- Use appropriate log levels: Use different log levels (DEBUG, INFO, WARN, ERROR) to categorize the severity and importance of log messages. This allows you to filter and analyze logs more effectively
- Use structured logging: Structured logging formats like JSON make it easier to parse and analyze log data
