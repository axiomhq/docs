...
All configuration happens through environment variables. The script below sets everything needed.
...
<Warning>
Source the script (`source ./setup-otel.sh`) rather than executing it directly. The exported variables must exist in the current shell where `claude` inherits them.
</Warning>
