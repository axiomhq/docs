...
- Maven: Use Maven as the build tool in this guide.
- Create an Axiom account.
- Create a dataset in Axiom where the data is sent.
...
`DiceRollerApp.java` is the core of the app. It simulates rolling a dice and demonstrates the usage of OpenTelemetry for tracing. The app includes two methods: one for a simple dice roll and another that demonstrates the usage of span links to establish relationships between spans across different traces.
...
The app executes the `rollDice()` and `rollDiceWithLink()` methods, generating telemetry data that’s sent to Axiom.

## Observe the telemetry data in Axiom

As the app runs, traces are collected and exported to Axiom. Monitor and analyze the app’s performance and behavior in the Axiom dashboard.
...
|                                       | telemetry.sdk.name                            | Name of the telemetry SDK, for exmaple., `opentelemetry`. |
