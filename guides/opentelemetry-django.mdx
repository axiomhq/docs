...
- Create a dataset in Axiom where the Django app sends telemetry data. For more information, see [Data settings](/docs/reference/datasets).
- Create an API key in Axiom with permissions to query and ingest data. For more information, see [Access Settings](/docs/reference/tokens).
...
3. Configure OpenTelemetry into your Django settings to capture telemetry data upon app startup.
...
4. Instrument views: Manually instrument views to create custom spans that trace specific operations within your Django app.
...
5. Instrument database queries: Apply manual tracing to database operations, wrapping database cursor executions with OpenTelemetry spans.
...
3. Initialize OpenTelemetry in Django: Ensure OpenTelemetry is initialized with Django to capture telemetry data from all HTTP requests automatically.
...
4. Update `manage.py`: Modify `manage.py` to include OpenTelemetry initialization, ensuring that tracing is active before the Django app fully starts.
...
5. (Optional) Add Custom Instrumentation: Combine automatic and custom manual spans in Django views to enhance trace details for specific complex operations.
