...
If you receive unauthorized errors when using Axiom MCP Server:

- **Verify your token type.** The remote MCP Server uses OAuth authentication. If you use the local setup or header-based authentication, ensure you use a [personal access token](/reference/tokens#personal-access-tokens-pat), not an API token. Personal access tokens and API tokens are different and not interchangeable for MCP authentication.
- **Check the organization ID.** If you use header-based authentication, ensure the `x-axiom-org-id` header contains the correct organization ID. For more information, see [Determine organization ID](/reference/tokens#determine-organization-id).
- **Clear cached credentials.** If you previously connected with different credentials, clear the `.mcp-auth` folder (default path on Mac: `~/.mcp-auth`) and reconnect.
- **Verify network access.** Ensure your environment can reach `https://mcp.axiom.co`. Some corporate networks, firewalls, or CI/CD environments like AWS DevOps agents may block outbound connections to this URL.

### Connection issues with CI/CD agents

AI agents running in CI/CD environments (such as AWS DevOps agents) may not support browser-based OAuth. In this case, use [header-based authentication](#header-based-authentication) with a personal access token. Set both the `Authorization` and `x-axiom-org-id` headers as described in the setup instructions.
