...
<Note>
Query results from this endpoint are routed through the US East 1 (AWS) deployment, regardless of which edge deployment holds your data. To query your edge data without results leaving the relevant region, use the [edge query endpoint](/restapi/endpoints/queryAplEdge) instead.
</Note>
