...
- Use `sort by _time asc` or `sort by _time desc` in the APL query. This returns the results in ascending or descending chronological order. For more information, see the [sort operator](/apl/tabular-operators/sort-operator).
- Specify `startTime` and `endTime` in the request body.

### Run Query (Legacy)

To use pagination with the legacy Run Query endpoint:
- Add the `limit` parameter to the request body. The value of this parameter determines the number of events to display per page.
- Add the `order` parameter to the request body. In the value of this parameter, order the results by time in either ascending or descending chronological order. For example, `[{ "field": "_time", "desc": true }]`. For more information, see the [order operator](/apl/tabular-operators/order-operator).
- Specify `startTime` and `endTime` in the request body.
...
To page through the result set, add the `cursor` parameter to the request body.
