...
The only required header is `Authorization: Bearer`, which is your API or personal token. Learn more about [API Token](/reference/tokens) and [Org ID](/restapi/introduction#organization-id).

## Using the Axiom JS library to ingest data

Axiom maintains [axiom-js](https://github.com/axiomhq/axiom-js) to provide official JavaScript bindings for the Axiom API.
...
These examples send an API event to Axiom. Before getting started with the Axiom API, you need to create a [Dataset](/reference/datasets) and [API Token](/reference/tokens).

## Ingest Events using JSON

The following example request contains grouped events. The structure of the JSON payload should follow the scheme `[ { "labels": { "key1": "value1", "key2": "value12" } }, ]`, where the array comprises one or more JSON objects describing events.
...
Dataset names are case-sensitive. They must be between 1-128 characters and may only contain ASCII alphanumeric characters and the '-' character.