...
description: "This page explains how to calculate and manage resources to optimize usage within Axiom."
...
On Axiom Cloud, Adaptive Tiering automatically moves your data between three access tiers based on how recently you queried it:

| Access tier | Data in this access tier |
|:------------|:-------------------------|
| Frequent Access | Newly ingested data, and any data queried in the last 30 days. |
| Infrequent Access | Data that hasn’t been queried for 30 consecutive days. |
| Archive Instant Access | Data that hasn’t been queried for 90 consecutive days. |

Every access tier delivers the same low latency and high throughput. You query data in Infrequent Access and Archive Instant Access exactly as you query data in Frequent Access: there’s no restore step, no rehydration, and no wait. The only thing that changes is the price. For the rates of each access tier, see the [Axiom pricing page](https://axiom.co/pricing).

Adaptive Tiering needs no configuration. When you query data in Infrequent Access or Archive Instant Access, it returns to Frequent Access, then moves down again after 30 and 90 consecutive days without a query. A query reads all data in its time range, so a broad time range returns more data to Frequent Access than a narrow one.

Because Axiom stores machine data in a highly compressed columnar block format, storage is typically a small proportion of an Axiom bill. Adaptive Tiering reduces it further the longer data goes unqueried, so keeping a long retention period costs less.

To see how much of your data is in each access tier, go to <Icon icon="gear" iconType="solid"/> **Settings > Usage**. For more information, see [Usage](/reference/usage-billing#usage).
...
1. **Query costs**: Monitor which queries and monitors consume the most resources.
1. **High-cost queries**: List queries that exceed a cost threshold.
...
### Set refresh rate for dashboards
