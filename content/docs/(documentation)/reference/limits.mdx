...
| Maximum buckets per histogram data point | 256                     |
| Maximum quantiles per summary data point | 256                     |
...
The maximum number of buckets applies to each histogram data point and counts the entries in its `explicit_bounds` list. The implicit overflow bucket that catches values above the last boundary doesn’t count towards the limit. For summary data points, the limit counts the entries in the `quantile_values` list. For scale, the default OpenTelemetry SDK histogram uses 15 boundaries. If your histograms exceed the limit, reduce the number of boundaries at the source. For example, configure a view in your OpenTelemetry SDK, or drop or filter the affected metrics in your OpenTelemetry Collector configuration.
