...
This guide sets `eventdb.wal.provider: cephfs` and names the CephFS class in `eventdb.wal.storageClass`. The chart creates nothing for it; it only claims from the class. [Deploy Ceph](/self-hosted/ceph#step-6-verify-before-installing-eventdb) proves the class binds a claim before you get here. If your cluster has a different RWX class, name its provider (`nfs` for an NFS export or a vendor filer, `efs` on AWS) and that class instead.

## Step 4: Choose object storage

EventDB stores every block in object storage, and `objectStorage.provider` names which: `ceph` for a Ceph RGW, `minio`, `seaweedfs`, `hetzner`, `s3` for AWS, or `gcs`. Provide:
...
MinIO, SeaweedFS, Hetzner Object Storage, or AWS S3 work the same way with an HTTPS endpoint and their own provider name (`minio`, `seaweedfs`, `hetzner`, `s3`), and need no `hostAliases`. The chart ships the AWS shape as an overlay, `values-s3.example.yaml`; `s3` is also the one provider whose endpoint the chart derives from the region.
...
- **`eventdb.wal.storageClass` names a class the chart never creates.** With `provider: cephfs` the render refuses an empty name, and a name that does not exist leaves the claim `Pending`. Deploy Ceph's Step 6 is what proves the class works.
