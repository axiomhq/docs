...
```bash
# storage nodes: one per failure domain
...
The dependency images are open source and public: Rook, Ceph, the Ceph CSI plugins, CloudNativePG and its PostgreSQL image, Valkey, and the `busybox` and `amazon/aws-cli` images the bootstrap Jobs run. They pull without credentials from their upstream registries, so leave the subcharts alone. Only a disconnected site that mirrored them into an authenticated registry needs to name the secret for each subchart as well (`rookCephOperator`, `rookCephCluster`, `cephCsiOperator` and `postgresqlOperator` take `imagePullSecrets`; the PostgreSQL cluster takes `postgresql.cluster.imagePullSecrets`; Valkey reads `global.imagePullSecrets`) and to patch the two CephFS CSI plugin accounts the chart renders, `cephfs-ctrlplugin-sa` and `cephfs-nodeplugin-sa`, after the install.

## Step 3: Provision local storage

Two kinds of storage are needed:

- **A block StorageClass.** Every stateful piece claims ReadWriteOnce volumes from it: Ceph's OSDs and monitors, PostgreSQL and its write-ahead log, and Valkey. Nothing in the release touches a disk directly. The OSDs are PVC-backed, so a lost pod comes back on a fresh, empty claim, and adding capacity is raising a count in the values rather than cabling a disk.
...
<Note>
**Durability.** EventDB's source of truth is object storage, replicated three ways across the OSD volumes. The shared WAL holds only what has not yet been flushed into a block, and the delta cache is rebuilt from it. PostgreSQL holds dataset metadata and Atom's management state and is the one datastore that needs [backups](/self-hosted/backup-restore).
</Note>

### Provide a block StorageClass

The class has two requirements beyond being fast:

- **It must support `volumeMode: Block`.** The OSD claims are raw block volumes that Ceph formats itself. Most CSI drivers do this; Rancher's `local-path` provisioner does not, so it can serve PostgreSQL and Valkey but not the OSDs.
- **It decides whether an OSD can move.** With a networked class (a SAN or cloud volumes), a volume can re-attach on another node and the device set runs with `portable: true`. With a node-local class (OpenEBS LocalPV, TopoLVM, Longhorn in strict-local mode), an OSD is pinned to the node that holds its volume; set `portable: false` and let `removeOSDsIfOutAndSafeToRemove` purge an OSD whose node is gone so Rook creates a fresh one elsewhere.

Capacity: the guide creates three OSD volumes of the size you set in Step 6 and one 10 GiB volume per monitor. Ceph replicates every object three times, so raw capacity is three times the compressed data you intend to keep, and Ceph should never fill past 80 %. PostgreSQL adds 50 GiB of data and 30 GiB of WAL per instance; Valkey 10 GiB per replica.
...
The RGW serves plain HTTP on its Service name, and EventDB only speaks plain HTTP to a hostname ending in `.localhost`. Step 5 reserves an address for it and Step 6 maps the name onto that address in each EventDB pod, so no cluster-wide DNS change is needed.
...
EventDB speaks plain HTTP only to a hostname ending in `.localhost`, and Rook publishes the RGW under its Service name, so the values file names the RGW as `ceph-rgw.eventdb.localhost` and maps that name onto an IP in each EventDB pod's `/etc/hosts`. The IP has to be known before the install, and Rook's own Service does not exist yet, so create a Service of your own in front of the RGW pods now. The API server assigns its ClusterIP immediately; it has no endpoints until Ceph is up, which is expected.
...
RGW_IP=$(kubectl -n $NAMESPACE get service ceph-rgw-alias -o jsonpath='{.spec.clusterIP}')
echo "$RGW_IP"
```

Keep that IP for Step 6. It is stable for the life of the Service, and the selector is Rook's own daemon labelling, unchanged across Rook 1.x.

On k3s, whose CoreDNS imports the `kube-system/coredns-custom` ConfigMap, the chart can do the mapping itself instead: skip the Service, enable `localDns` in Step 6, and leave `hostAliases` empty.
...
objectStorage:
  ...
  # eventdb.hostAliases below maps it onto the RGW.
...
eventdb:
  enabled: true
  environment: prod
  # The ClusterIP of the ceph-rgw-alias Service from Step 5. Rendered into
  # every EventDB pod's /etc/hosts; nothing outside the release sees it.
  hostAliases:
    - ip: <RGW_IP>
      hostnames:
      ...
      volumeClaimTemplate:
        spec:
          ...
          resources:
            requests:
              storage: 10Gi
    # An OSD whose node is gone is purged once its data is safe elsewhere,
    # and the device set below creates a fresh one to replace it.
    removeOSDsIfOutAndSafeToRemove: true
    storage:
      useAllNodes: false
      useAllDevices: false
      # PVC-backed OSDs: three raw block volumes from your class, one per
      # node. No disks are listed or wiped; a lost OSD comes back on an
      # empty claim. portable: true for a networked class; false for a
      # node-local one (see Step 3).
...
Helm returns once the resources exist. What follows takes longer: Rook prepares the OSDs, the operators bring up their data planes, and the EventDB workloads wait behind the bootstrap Jobs and, until the RGW answers, restart on their object-storage connection, which is expected.
...
Rook runs one prepare Job per OSD claim, and the claims bind first; the whole thing takes several minutes on first boot:
...
The `eventdb-wal` claim binds on its own once the filesystem's metadata server is up. If an OSD is missing after ten minutes, see [Troubleshoot](/self-hosted/troubleshoot#an-osd-claim-stays-pending-or-no-osd-appears).
