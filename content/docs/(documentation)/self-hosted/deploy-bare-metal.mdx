...
If your registry requires authentication for pulls, create the release namespace and a pull secret now:
...
Then name it in the values file you write in Step 6. `global.imagePullSecrets` reaches every first-party pod, including the bootstrap Jobs, CronJobs and `helm test` pods, and any subchart that reads a Helm global. The Rook and CloudNativePG charts read only their own key, so list it for them as well:
