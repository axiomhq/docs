...
<Warning>
Put the bucket outside the cluster. A Ceph RGW on the same cluster shares fate with the PostgreSQL volumes, which are likely on the same Ceph, and a backup that dies with the thing it protects is not one. Use a second site, a cloud bucket, or a separate object store.
</Warning>
