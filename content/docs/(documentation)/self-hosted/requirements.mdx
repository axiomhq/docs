...
| Role | Count | Purpose |
| :--- | :--- | :--- |
| Storage | 3 or more | Ceph monitors, managers, and OSDs. Their volumes come from the block StorageClass; no node holds a disk of its own. |
...
### Block volumes for Ceph

When the chart manages object storage, Rook runs PVC-backed OSDs: `rookCephCluster.cephClusterSpec.storage.storageClassDeviceSets` names a count and a size, and each OSD claims a raw block volume (`volumeMode: Block`) from your StorageClass. The monitors claim 10 GiB filesystem volumes the same way. Nothing lists, formats, or wipes a disk, and a lost OSD is replaced by a fresh, empty claim. Requirements:

- A StorageClass that supports `volumeMode: Block`. Most CSI drivers do; Rancher's `local-path` provisioner does not.
- `portable: true` with a networked class, whose volumes re-attach on another node; `portable: false` with a node-local class, where an OSD is pinned to its node and `removeOSDsIfOutAndSafeToRemove` lets Rook purge one whose node is gone.
- NVMe or SSD behind the class. Ceph's write path fsyncs; slow volumes become slow ingest.
- At least three OSDs on three nodes, so the three-way replicated pools can place every copy on a different host.
...
PostgreSQL data and WAL volumes, Valkey persistence, and NATS (if used) claim ReadWriteOnce volumes from a StorageClass you name in the values, and so do the Ceph OSDs and monitors above. One class can serve all of them if it supports block mode: OpenEBS LocalPV, TopoLVM, Longhorn, or any CSI driver over a SAN or cloud volumes. A filesystem-only class such as `local-path` can still serve PostgreSQL and Valkey beside a block-capable one for Ceph.
...
<Note>
EventDB only speaks plain HTTP to an endpoint whose hostname is `localhost`, `127.0.0.1`, `::1`, or a `*.localhost` name. Every other hostname must use TLS. An in-cluster RGW answers on its Service name over HTTP, so a `*.localhost` alias is mapped onto it: by the chart's CoreDNS rewrite on k3s, or through `eventdb.hostAliases` pointed at a Service you create in front of the RGW pods before the install. The bare-metal guide does this in [Step 5](/self-hosted/deploy-bare-metal#reserve-the-object-storage-address).
</Note>
