...
Every change to the release, whether a values edit, new EventDB or Atom images, or a new chart version, is a `helm upgrade` of the same release with the same values file. What that does, the pre-flight checks, the ordering for CloudNativePG upgrades, the changes that need more than an upgrade, and how to roll back are on their own page: [Upgrade](/self-hosted/upgrade). The object store and the shared filesystem are outside the release; a Ceph's day-two care is on [Deploy Ceph](/self-hosted/ceph#operate).

<Warning>
**Never disable a data-plane section on an upgrade.** Setting `postgresql.enabled`, `valkey.enabled`, or `eventdb.enabled` to `false` on a running release deletes the PostgreSQL cluster, the cache, or the EventDB workloads, with their data.
</Warning>
...
- **Object storage.** It is yours to grow. On Ceph, watch raw usage and add OSDs before it passes 80%, by labelling another node as [Deploy Ceph](/self-hosted/ceph#operate) describes; Ceph rebalances onto new OSDs automatically.
- **Shared WAL.** The CephFS class from Deploy Ceph allows volume expansion; edit `eventdb.wal.size` upward and upgrade. It cannot shrink. Another RWX class expands only if it allows it.
...
The EventDB switch scrapes `/metrics` on ingest, query runner, delta cache, dataset manager, queue worker, and the compaction scheduler. Atom's switch scrapes its `/metrics`. Ceph's metrics and Rook's maintained health and capacity alerts are switched on where Ceph is deployed; see [Deploy Ceph](/self-hosted/ceph#operate).
...
| Object-store health, on Ceph `HEALTH_OK` and raw usage under 75% | The object store is the ceiling for everything. |
