...
**Fix.** Name the class. On bare metal that is `eventdb.wal.provider: existing` with `eventdb.wal.storageClass` set to the CephFS class from [Deploy Ceph](/self-hosted/ceph), as in the deployment guide. The class must exist and serve `ReadWriteMany` before the install; the chart creates nothing for it.
