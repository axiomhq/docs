...
This section covers a bare-metal deployment end to end. Start with [Requirements](/self-hosted/requirements), deploy the object store and shared filesystem the chart consumes, for which [Deploy Ceph for EventDB](/self-hosted/ceph) is the validated path, then follow [Deploy on bare metal](/self-hosted/deploy-bare-metal).
...
flowchart LR
    ...
    subgraph release [axiom-eventdb release]
        ...
        wal[(Shared WAL<br/>RWX volume from your class)]
        pg[(PostgreSQL<br/>CloudNativePG)]
        valkey[(Valkey)]
    end

    s3[(Object storage<br/>S3-compatible: Ceph RGW, MinIO, S3)]
...
| Shared WAL on an object-storage bucket | Optional. The JuiceFS CSI driver, for clusters with no shared filesystem | `juicefs/juicefs-csi-driver` `0.32.5` when selected |

Every dependency is pinned to an exact chart version and to the SHA-256 of its archive. The CloudNativePG CRDs ship in a companion subchart so a release can create its PostgreSQL cluster in the same Helm transaction that installs the operator.

Object storage and the shared write-ahead log's ReadWriteMany volume are not dependencies. The chart consumes an S3-compatible endpoint and a `StorageClass` that the cluster already provides. Ceph provides both, through its RGW gateway and CephFS, and is the path Axiom validates on bare metal; deploying it is described in [Deploy Ceph for EventDB](/self-hosted/ceph).
...
- A conformant Kubernetes cluster. See [Requirements](/self-hosted/requirements).
- An S3-compatible object store for EventDB's blocks: a Ceph RGW, MinIO, AWS S3, or Google Cloud Storage.
- A ReadWriteMany `StorageClass` for the shared WAL: CephFS, NFS, GKE Filestore, or JuiceFS through the chart's optional subchart.
- A block `StorageClass` from which PostgreSQL and Valkey claim their volumes.
...
| [Deploy Ceph for EventDB](/self-hosted/ceph) | What EventDB needs from a Ceph you operate: the RGW endpoint, the CephFS class, topology, devices, credentials, and day-two care. |
