...
keywords: ['self-hosted', 'eventdb', 'upgrade', 'helm upgrade', 'rollback', 'crd', 'cloudnativepg', 'migration', 'ceph', 'atom']
---
...
5. **The operator reconciles its data plane.** Changes under `postgresql` are applied to the `Cluster` resource; CloudNativePG then carries them out on its own schedule, outside the Helm transaction. The object store and the shared filesystem are outside the release altogether; a change to a Ceph is a change to its own releases, see [Deploy Ceph](/self-hosted/ceph#operate).

Two things an upgrade never does:

- **It never upgrades CRDs.** Helm installs the files under a chart's `crds/` directory on first install and deliberately leaves them alone afterwards. A chart release that bumps CloudNativePG needs its CRDs applied by hand first. See [Operator upgrades](#operator-upgrades).
...
<Warning>
**Never disable a data-plane section on an upgrade.** Setting `postgresql.enabled`, `valkey.enabled`, or `eventdb.enabled` to `false` on a running release deletes the PostgreSQL cluster, the cache, or the EventDB workloads, with their data. Those toggles are install-time decisions, and there is no reason to turn them off on a running release.
</Warning>
...
| Chart version with a new operator or dependency | CRDs, the operator, then its data plane under the operator's control | Rolling for PostgreSQL with a primary switchover |
...
kubectl -n rook-ceph get cephcluster                         # HEALTH_OK, if Ceph is on this cluster
```

CloudNativePG needs a healthy standby to switch the primary over to, and an object store that is degraded turns a rolling ingest restart into rejected batches. Fix health first.
...
Only for a chart release whose notes say CloudNativePG was bumped. See [Operator upgrades](#operator-upgrades) for the full sequence; the CRD step is:
...
`helm show crds` prints the CRDs from the chart and from its companion CRD subchart, which is where the pinned CloudNativePG definitions live.
...
The chart pins CloudNativePG exactly, so a chart release that moves it is an operator upgrade. CloudNativePG documents an order, and a single Helm transaction does not enforce it. Follow the upstream ordering. A Ceph upgrade is not a chart upgrade at all: it happens on the Rook releases, operator before cluster, and is described on [Deploy Ceph](/self-hosted/ceph#operate). Ceph stays available throughout a healthy upgrade, and so do ingest and queries.
...
**Switching object stores.** Nothing migrates the objects. A release pointed at a different endpoint or bucket starts with an empty bucket; every existing dataset's blocks stay where they were and are unreachable. Treat this as a new deployment with a data migration you plan separately.
...
The one exception is size on a class that allows expansion, such as the CephFS class from [Deploy Ceph](/self-hosted/ceph): edit the claim's `spec.resources.requests.storage` directly with `kubectl`, then set the same `eventdb.wal.size` in the values so the chart agrees with the cluster.

## From a chart that deployed Ceph

Chart versions before `0.2.0` bundled Rook and Ceph as subcharts, selected with `objectStorage.provider: rook`, `eventdb.wal.provider: cephfs` deriving its class from the subchart, and the `rookCephOperator`, `cephCsiOperator`, and `rookCephCluster` values. From `0.2.0` the chart deploys no Ceph: the S3 endpoint and the ReadWriteMany class are prerequisites you provide, as [Deploy Ceph](/self-hosted/ceph) describes, both providers name the store and the filesystem you connected, and the chart's schema rejects the three removed blocks.

<Warning>
**Do not `helm upgrade` a release that had Rook enabled to `0.2.0`.** Helm removes every resource the old chart rendered and the new one does not, which includes the `CephCluster`, its object store, and its filesystem. Rook then tears the Ceph cluster down, with EventDB's blocks and WAL in it.
</Warning>

Treat it as a new deployment. Deploy Ceph on its own, as its own Rook releases, migrate the blocks and the PostgreSQL data across in a plan of your own, and install the chart fresh against the new endpoint and class with `objectStorage.provider: ceph` and `eventdb.wal.provider: cephfs` naming them.
...
- **CRDs.** Helm does not touch them in either direction. Rolling an operator back to a version whose CRDs lack fields the newer operator wrote is unsupported by CloudNativePG; roll operators forward, not back.
