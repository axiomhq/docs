...
Treat it as a new deployment. Deploy Ceph on its own, as its own Rook releases, migrate the blocks and the PostgreSQL data across in a plan of your own, and install the chart fresh against the new endpoint and class with `objectStorage.provider: external` and `eventdb.wal.provider: existing`.
