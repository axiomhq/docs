...
You can select a visualization to add it to the query. If a visualization requires an argument (such as the field and/or other parameters), the menu allows you to select eligible fields and input those arguments. Press <kbd className="inline-flex h-5 items-center justify-center rounded-sm border border-border bg-muted px-1.5 align-middle font-mono text-xs leading-none font-medium text-muted-foreground">Enter</kbd> to complete the addition.
