---
title: 'Set up standard mode federated search'
description: 'Learn how to register Axiom as a Splunk federated provider in standard mode and map Axiom datasets as federated indexes.'
sidebarTitle: Set up standard mode
keywords: ['splunk', 'federated search', 'federated provider', 'federated index', 'standard mode', 'setup']
---

In standard mode, each Axiom dataset you expose is mapped to a Splunk federated index, and users search it as `index=federated:<name>`. Standard mode is explicit, simple to reason about, and the recommended starting point. For how the modes differ, see [How federated search works](/splunk/federated-search/overview).

<Note>
If you use Splunk Enterprise Security, [set up transparent mode](/splunk/federated-search/set-up-transparent) instead. Splunk doesn’t support federated search in standard mode with Enterprise Security.
</Note>
...
## Add Axiom as a federated provider
...
If either search shows an error banner, see [Monitor and troubleshoot](/splunk/federated-search/troubleshoot).

## What’s next

- [SPL command support](/splunk/federated-search/spl-support)
- [Examples](/splunk/federated-search/examples)
- Want your Splunk lookups and data models to work against Axiom data? [Set up transparent mode](/splunk/federated-search/set-up-transparent).
