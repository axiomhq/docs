---
title: 'SPL command support in federated search'
...
keywords: ['splunk', 'spl', 'federated search', 'pushdown', 'stats', 'timechart', 'tstats', 'command support', 'limits']
---

Every SPL command falls into one of three states when it runs against Axiom data. This is the same model Splunk uses between its own deployments.
...
Transparent mode providers replicate the search head’s knowledge bundle to Axiom, which evaluates the objects a provider must. Everything else arrives pre-expanded by Splunk.
...
There are two genuine limits, and both come from outside Axiom. Everything else is either a bounded estimate that warns when reached, or standard Splunk behavior.
