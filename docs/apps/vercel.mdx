...
If you use Axiom Vercel integration, [annotations](/docs/query-data/annotate-charts) are automatically created for deployments.
...
Simply install the [Axiom Vercel app from here](https://vercel.com/apps/axiom) and be streaming logs and web vitals within minutes!
...
In May 2024, Vercel [introduced higher costs](/blog/changes-to-vercel-log-drains) for using Vercel Log Drains. Because the Axiom Vercel app depends on Log Drains, using the next-axiom library can be the cheaper option to analyze telemetry data for higher volume projects.
...
2. Delete `NEXT_PUBLIC_AXIOM_INGEST_ENDPOINT` from the environment variables of your Vercel project. For more information, see the [Vercel documentation](https://vercel.com/docs/projects/environment-variables).
3. [Create a new dataset in Axiom](/docs/reference/datasets), and [create a new advanced API token](/docs/reference/tokens) with ingest permissions for that dataset.
