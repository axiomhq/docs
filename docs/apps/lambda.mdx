...
excerpt: 'Send Lambda function logs and platform events to Axiom using the quick filters dashboard.'
tags:
...
- In Axiom UI, create a [dataset](/docs/usage/settings#dataset) and an [API Token](/docs/usage/settings#token) with ingest permissions for that dataset.
...
![Add new layer](/doc-assets/shots/add-new-layer.png)
...
![Adding environment variables](/doc-assets/shots/add-env-vars.png)

2. On the **Edit environment variables** page, click **Add environment variable** to specify your dataset name and an API token:

![Added environment variables](/doc-assets/shots/added-env-vars.png)
...
![Disable CloudWatch logging](/doc-assets/shots/disable-cw-logs.png)