---
title: 'Splunk to APL Conversion Reference'
description: 'This step-by-step guide provides a high-level mapping from Splunk to APL.'
...
Splunk uses the `table` command to select which columns to include in the results. APL has a `project` operator that does the same and [more](https://axiom.co/docs/apl/tabular-operators/project-operator).
...
See the [list of summarize aggregations functions](https://axiom.co/docs/apl/aggregation-function/statistical-functions) that are available.
