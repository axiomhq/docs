...
Splunk uses the `table` command to select which columns to include in the results. APL has a `project` operator that does the same and [more](/docs/apl/tabular-operators/project-operator).
...
See the [list of summarize aggregations functions](/docs/apl/aggregation-function/statistical-functions) that are available.
