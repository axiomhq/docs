...
If you get a 400 when you use the field name `_time` with the [Elastic Search endpoint](/docs/send-data/elastic-beats.mdx) or when you override the [`timestamp` field](/docs/reference/field-restrictions), use the query parameter `?timestamp-field` to set a field as the time field.
...
[Learn about supported visualizations](/docs/query-data/visualizations)
