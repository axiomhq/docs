---
title: 'Send Tremor logs to Axiom'
excerpt: 'This step-by-step guide will help you configure Tremor connectors and events components to interact with your databases, APIs, and ingest data from these sources into Axiom.'
tags:
  ['axiom documentation', 'documentation', 'axiom', 'guide', 'tremor', 'http', 'syslog', 'connectors', 'sink', 'json', 'tcp client', 'syslog forwarder', 'host port']
---
...
- The`$DATASET_NAME` [dataset](https://axiom.co/docs/reference/settings#dataset) you want to send logs to in Axiom, and the `$API_TOKEN` is your [Axiom API token](https://axiom.co/docs/reference/settings#api-token) for ingesting and quering your Tremor logs.



## Configuration using Syslog

You can also send logs via Tremor to the [Axiom Syslog endpoint](https://axiom.co/docs/endpoints/introduction), if you're on the [basic plan](https://axiom.co/pricing) (using a file as an example data source).
...
- Replace the `url` with the URL of your [Syslog endpoint](https://axiom.co/docs/endpoints/introduction) generated from **Settings --> Endpoints --> New endpoint** in the Axiom UI.