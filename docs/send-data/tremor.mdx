...
The`$DATASET_NAME` [dataset](/docs/reference/datasets) you want to send logs to in Axiom, and the `$API_TOKEN` is your [Axiom API token](/docs/reference/tokens) for ingesting and quering your Tremor logs.

## Configuration using Syslog

You can also send logs via Tremor to the [Axiom Syslog endpoint](/docs/endpoints/introduction) using a file as an example data source.
...
Replace the `url` with the URL of your [Syslog endpoint](/docs/endpoints/introduction) generated from **Settings > Endpoints > New endpoint** in the Axiom UI.