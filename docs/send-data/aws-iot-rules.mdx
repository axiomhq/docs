...
def lambda_handler(event, context):
    ...
    headers = {
        "Authorization": f"Bearer {ingest_token}",  # Set the Authorization header with the token
...
- `INGEST_TOKEN` is the Axiom API token you have generated. For added security, store the API token in an environment variable.
